...
The [SimVascular svFSIplus Documention](https://simvascular.github.io/documentation/svfsi.html) providess documentation describing how to use the svFSIplus solver. It also has developer guide describing the code organization and some implementation details. 

The [svFSIplus Internal Code Documentation](https://ktbolt.github.io/sficpp/index.html) provides documentation of the svFSIplus source code. It is autotmatically generated using [Doxygen](https://www.doxygen.nl).
...
The following steps demonstate how to build local VTK libraies in the `/user/shared/vtk` directory from the source downloaded from https://www.vtk.org/files/release/9.3/VTK-9.3.1.tar.gz. 
...
These commands will create the following directorties under /user/shared/vtk/install
```
bin/ include/ lib/ share/
```

You can then build svFSplus with the local VTK libraries using the two CMake command-line arguments that sets the option to use a local VTK build and the location of the build
```
...
Once the **svfsiplus** binary is available add its location to your environtment variable (`PATH` on Unix-like systems).
...
The simulation progresss for each time step will be printed showing
...
```
In this case a directory named `4-procs` containing the simulation results output will be created. Results from different processors will be conbined into a single file for a given time step.

<!--- =================================================================================== -->
<!---                             Docker container                                        -->
<!--- =================================================================================== -->

<h1 id="docker_container">  Docker Containe </h1>
