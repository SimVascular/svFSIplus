...
class Vector
{
  public:
    ...
    {
      // [NOTE] This is tfu but need to mimic Fortran that can allocate 0-sized
      // arrays.
      if (size <= 0) {
        is_allocated_ = true;
        return;
    ...
    bool allocated() const { return is_allocated_; }
    ...
    /// @brief This class provides an interface to access Vector like STL
    /// containers.
    //
    class Iterator
    {
    ...
    {
      if (data_ == nullptr) {
        std::cout << "[Vector] WARNING: Accessing null data in Vector at " << i
                  << std::endl;
        return;
      ...
      if ((i < 0) || (i >= size_)) {
        ...
        throw std::runtime_error(+"Index i=" + index_str +
                                 " is out of bounds for " + dims + " vector.");
      }
    }
    ...
    void check_type() const
    {
      if (!std::is_same<T, double>::value && !std::is_same<T, int>::value &&
          !std::is_same<T, Vector<double>>::value &&
          !std::is_same<T, float>::value) {
        std::string msg =
            std::string("Cannot use Vector class template for type '") +
            typeid(T).name() + "'.";
