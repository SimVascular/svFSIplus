...
double mat_det(const Array<double>& A, const int nd) {
  ...
  if (nd == 2) {
    ...
    for (int i = 0; i < nd; i++) {
      ...
      D = D +
          pow(-1.0, static_cast<double>(2 + i)) * A(0, i) * mat_det(Am, nd - 1);
    }
  }
...
Array<double> mat_inv_ge_orig(const Array<double>& A, const int nd,
  ...
  for (int i = 0; i < nd; i++) {
    if (debug) {
      std::cout << "[mat_inv] ========== i " << i + 1
                << " ==========" << std::endl;
    }
    for (int j = 0; j < nd; j++) {
      if (debug) {
        std::cout << "[mat_inv] ########## j " << j + 1 << " ##########"
                  << std::endl;
      }
      if (j != i) {
        d = B(j, i) / B(i, i);
...
Array<double> mat_mul(const Array<double>& A, const Array<double>& B) {
  ...
  if (A_num_cols != B_num_rows) {
    throw std::runtime_error("[mat_mul] The number of columns of A (" +
                             std::to_string(A_num_cols) + ") does not equal " +
  ...
  if (nd == 2) {
    ...
    for (int ii = 0; ii < nn; ii++) {
      ...
      C(i, j, k, l) =
          ...
          A(i, j, 0, 1) * B(k, l, 0, 1) + A(i, j, 0, 2) * B(k, l, 0, 2) +
          A(i, j, 1, 0) * B(k, l, 1, 0) + A(i, j, 1, 1) * B(k, l, 1, 1) +
          A(i, j, 1, 2) * B(k, l, 1, 2) + A(i, j, 2, 0) * B(k, l, 2, 0) +
          A(i, j, 2, 1) * B(k, l, 2, 1) + A(i, j, 2, 2) * B(k, l, 2, 2);
    }
  }
  ...
  if (nd == 2) {
    ...
    for (int ii = 0; ii < nn; ii++) {
      ...
      C(i, j, k, l) =
          ...
          A(i, 0, j, 1) * B(0, 1, k, l) + A(i, 0, j, 2) * B(0, 2, k, l) +
          A(i, 1, j, 0) * B(1, 0, k, l) + A(i, 1, j, 1) * B(1, 1, k, l) +
          A(i, 1, j, 2) * B(1, 2, k, l) + A(i, 2, j, 0) * B(2, 0, k, l) +
          A(i, 2, j, 1) * B(2, 1, k, l) + A(i, 2, j, 2) * B(2, 2, k, l);
    }
  }
  ...
  if (nd == 2) {
    ...
    for (int ii = 0; ii < nn; ii++) {
      ...
      C(i, j, k, l) =
          ...
          A(i, j, 0, 1) * B(k, 0, l, 1) + A(i, j, 0, 2) * B(k, 0, l, 2) +
          A(i, j, 1, 0) * B(k, 1, l, 0) + A(i, j, 1, 1) * B(k, 1, l, 1) +
          A(i, j, 1, 2) * B(k, 1, l, 2) + A(i, j, 2, 0) * B(k, 2, l, 0) +
          A(i, j, 2, 1) * B(k, 2, l, 1) + A(i, j, 2, 2) * B(k, 2, l, 2);
    }
  }
...
Array<double> ten_mddot(const Tensor4<double>& A, const Array<double>& B,
  ...
  if (nd == 2) {
    for (int i = 0; i < nd; i++) {
      for (int j = 0; j < nd; j++) {
        C(i, j) = A(i, j, 0, 0) * B(0, 0) + A(i, j, 0, 1) * B(0, 1) +
                  A(i, j, 1, 0) * B(1, 0) + A(i, j, 1, 1) * B(1, 1);
      }
    }
    ...
    for (int i = 0; i < nd; i++) {
      for (int j = 0; j < nd; j++) {
        C(i, j) = A(i, j, 0, 0) * B(0, 0) + A(i, j, 0, 1) * B(0, 1) +
                  A(i, j, 0, 2) * B(0, 2) + A(i, j, 1, 0) * B(1, 0) +
                  A(i, j, 1, 1) * B(1, 1) + A(i, j, 1, 2) * B(1, 2) +
                  A(i, j, 2, 0) * B(2, 0) + A(i, j, 2, 1) * B(2, 1) +
                  A(i, j, 2, 2) * B(2, 2);
      }
    }
  }
