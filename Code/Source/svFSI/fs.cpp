  ...
  if (lStab) {
    ...
    if (iOpt == 1) {
      ...
      for (int g = 0; g < fs[1].nG; g++) {
        nn::get_gnn(nsd, fs[1].eType, fs[1].eNoN, g, fs[1].xi, fs[1].N,
                    fs[1].Nx);
      }
      ...
      for (int g = 0; g < fs[0].nG; g++) {
        nn::get_gnn(nsd, fs[0].eType, fs[0].eNoN, g, fs[0].xi, fs[0].N,
                    fs[0].Nx);
      }
...
#define n_debug_init_fs_face
  ...
  if (lFa.eType != ElementType::NRB) {
    ...
    lFa.fs[0].N = lFa.N;
    lFa.fs[0].Nx = lFa.Nx;
    nn::get_nn_bnds(nsd, lFa.fs[0].eType, lFa.fs[0].eNoN, lFa.fs[0].xib,
                    lFa.fs[0].Nb);
  }
  ...
  if (lFa.nFs == 2) {
    ...
    for (int g = 0; g < lFa.fs[1].nG; g++) {
      nn::get_gnn(insd, lFa.fs[1].eType, lFa.fs[1].eNoN, g, lFa.fs[1].xi,
                  lFa.fs[1].N, lFa.fs[1].Nx);
    }

    nn::get_nn_bnds(nsd, lFa.fs[1].eType, lFa.fs[1].eNoN, lFa.fs[1].xib,
                    lFa.fs[1].Nb);
  }
}
...
#define n_debug_init_fs_msh
  ...
  bool flag = (lM.eType == ElementType::HEX20) ||
              (lM.eType == ElementType::HEX27) ||
              (lM.eType == ElementType::WDG);
  if (flag) {
    std::cout << "WARNING: Second derivatives are not computed for "
                 "HEX20/HEX27/WDG type elements";
    // throw std::runtime_error(" Second derivatives are not computed for
    // HEX20/HEX27/WDG type elements");
  }
  ...
  if (!lM.fs[0].lShpF) {
    ...
    for (int g = 0; g < lM.fs[0].nG; g++) {
      nn::get_gn_nxx(insd, ind2, lM.fs[0].eType, lM.fs[0].eNoN, g, lM.fs[0].xi,
                     lM.fs[0].Nxx);
    }
  }
