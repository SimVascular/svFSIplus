      ...
      switch (cPhys) {
        ...
        default:
          throw std::runtime_error(
              "[b_assem_neu_bc] Undefined physics selection for assembly");
      }
    }
...
#define n_debug_b_neu_folw_p
  ...
  for (int e = 0; e < lFa.nEl; e++) {
    ...
    for (int g = 0; g < lFa.nG; g++) {
      ...
      if (cPhys == EquationType::phys_ustruct) {
        if (nsd == 3) {
          ustruct::b_ustruct_3d(com_mod, eNoN, w, N, Nx, dl, hl, nV, lR, lK,
                                lKd);
        } else {
          ustruct::b_ustruct_2d(com_mod, eNoN, w, N, Nx, dl, hl, nV, lR, lK,
                                lKd);
        }
...
#define n_debug_global_eq_assem
  ...
  switch (eq.phys) {
    ...
    default:
      throw std::runtime_error(
          "[global_eq_assem] Undefined physics selection for assembly");
  }
