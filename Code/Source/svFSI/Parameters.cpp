...
//
// The class methods defined here are used to process svFSIplus simulation
...
void Parameters::read_xml(std::string file_name) {
  ...
  if (root_element == nullptr) {
    throw std::runtime_error(
        "The following error occured while reading the XML file '" + file_name +
...
void BodyForceParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (smesh == nullptr) {
    throw std::runtime_error(
        "No MESH given in the XML <Add_BF mesh=MESH> element.");
  }
...
void BoundaryConditionRCRParameters::set_values(
  ...
  using std::placeholders::_1;
  using std::placeholders::_2;
  std::function<void(const std::string&, const std::string&)> ftpr = std::bind(
      &BoundaryConditionRCRParameters::set_parameter_value, *this, _1, _2);
  xml_util_set_parameters(ftpr, xml_elem, error_msg);
...
void BoundaryConditionParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (sname == nullptr) {
    throw std::runtime_error(
        "No NAME given in the XML <Add_BC name=NAME> element.");
  }
...
void HolzapfelGasserOgdenParameters::set_values(
  ...
  using std::placeholders::_2;
  std::function<void(const std::string&, const std::string&)> ftpr = std::bind(
      &HolzapfelGasserOgdenParameters::set_parameter_value, *this, _1, _2);
...
void ConstitutiveModelParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (stype == nullptr) {
    throw std::runtime_error(
        "No TYPE given in the XML <Constitutive_model type=TYPE> element.");
  }
...
CoupleCplBCParameters::CoupleCplBCParameters() {
  ...
  set_parameter("File_name_for_0D_3D_communication", "", required,
                file_name_for_0D_3D_communication);
  set_parameter("File_name_for_saving_unknowns", "", required,
                file_name_for_saving_unknowns);
  set_parameter("Number_of_unknowns", 0, required, number_of_unknowns);
  set_parameter("Number_of_user_defined_outputs", 0, required,
                number_of_user_defined_outputs);
  set_parameter("Unknowns_initialization_file_path", "", !required,
                unknowns_initialization_file_path);
...
void CoupleCplBCParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (stype == nullptr) {
    throw std::runtime_error(
        "No TYPE given in the XML <Stimulus=TYPE> element.");
  }
  type.set(std::string(stype));
  auto item = xml_elem->FirstChildElement();
...
void CoupleGenBCParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (stype == nullptr) {
    throw std::runtime_error(
        "No TYPE given in the XML <Couple_to_genBC type=TYPE> element.");
  }
  type.set(std::string(stype));
  auto item = xml_elem->FirstChildElement();
...
void VariableWallPropsParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (sname == nullptr) {
    throw std::runtime_error(
        "No TYPE given in the XML <Variable_wall_properties mesh_name=NAME> "
        "element.");
  }
  ...
  std::function<void(const std::string&, const std::string&)> ftpr = std::bind(
      &VariableWallPropsParameters::set_parameter_value, *this, _1, _2);
...
using SetViscosityParamMapType =
    std::map<std::string, std::function<void(VpType, XmlType)>>;
SetViscosityParamMapType SetViscosityModelParamsMap = {
    {ViscosityParameters::CARREAU_YASUDA_MODEL,
     [](VpType vp, XmlType params) -> void {
...
using PrintViscosityParamaMapType =
    std::map<std::string, std::function<void(VpType)>>;
PrintViscosityParamaMapType PrintViscosityModelParamsMap = {
    {ViscosityParameters::CARREAU_YASUDA_MODEL,
     [](VpType vp) -> void { vp->carreau_yasuda_model.print_parameters(); }},
    {ViscosityParameters::CASSONS_MODEL,
     [](VpType vp) -> void { vp->cassons_model.print_parameters(); }},
    {ViscosityParameters::CONSTANT_MODEL,
     [](VpType vp) -> void { vp->newtonian_model.print_parameters(); }},
};
...
void ViscosityNewtonianParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  using std::placeholders::_2;
  std::function<void(const std::string&, const std::string&)> ftpr = std::bind(
      &ViscosityNewtonianParameters::set_parameter_value, *this, _1, _2);
...
void ViscosityCarreauYasudaParameters::set_values(
    tinyxml2::XMLElement* xml_elem) {
  std::string error_msg =
  ...
  using std::placeholders::_2;
  std::function<void(const std::string&, const std::string&)> ftpr = std::bind(
      &ViscosityCarreauYasudaParameters::set_parameter_value, *this, _1, _2);
...
void ViscosityCassonsParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  using std::placeholders::_2;
  std::function<void(const std::string&, const std::string&)> ftpr = std::bind(
      &ViscosityCassonsParameters::set_parameter_value, *this, _1, _2);
...
void ViscosityParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (smodel == nullptr) {
    throw std::runtime_error(
        "No MODEL given in the <Viscosity model=MODEL > XML element.");
  }
...
void DomainParameters::set_values(tinyxml2::XMLElement* domain_elem) {
  ...
  /*
    ...
    == 0) { throw std::runtime_error("Unknown constitutive model '" +
    constitutive_model.value_ + "' for '" + constitutive_model.name_ +
        "' in '" + domain_params->Name() + "'.");
    }

    if (Parameters::equation_names.count(equation.value()) == 0) {
      throw std::runtime_error("Unknown equation name '" + equation.value() + "'
    for '" + equation.name_ +
        "' in '" + domain_params->Name() + "'.");
    }
  */
...
void FiberReinforcementStressParameters::set_values(
  ...
  std::function<void(const std::string&, const std::string&)> ftpr = std::bind(
      &FiberReinforcementStressParameters::set_parameter_value, *this, _1, _2);
...
void ContactParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (mname == nullptr) {
    throw std::runtime_error(
        "No MODEL given in the XML <Contact model=MODEL> element.");
  }
...
void EquationParameters::set_values(tinyxml2::XMLElement* eq_elem) {
  ...
  while (item != nullptr) {
    auto name = std::string(item->Value());
    // std::cout << "[EquationParameters::set_values] name: " << name <<
    // std::endl;
    ...
    if (name == BodyForceParameters::xml_element_name_) {
      ...
      try {
        ...
        try {
          ...
          throw std::runtime_error("Unknown " + xml_element_name_ +
                                   " XML element '" + name + ".");
        }
      }
...
void GeneralSimulationParameters::set_values(
...
void RemesherParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (stype == nullptr) {
    throw std::runtime_error(
        "No TYPE given in the XML <Remesher type=TYPE> element.");
  }
  type.set(std::string(stype));
  values_set_ = true;
  ...
  while (item != nullptr) {
    ...
    if (name == "Max_edge_size") {
      ...
      if (name == nullptr) {
        throw std::runtime_error(
            "No NAME given in the XML Remesher <Max_edge_size name=NAME  "
            "value=VALUE> element.");
      }
      ...
      if (value == nullptr) {
        throw std::runtime_error(
            "No VALUE given in the XML Remesher <Max_edge_size name=NAME  "
            "value=VALUE> element.");
      }
...
void ProjectionParameters::set_values(tinyxml2::XMLElement* xml_elem) {
  ...
  if (sname == nullptr) {
    throw std::runtime_error(
        "No TYPE given in the XML <Add_projection name=NAME> element.");
  }
  name.set(std::string(sname));
...
LinearSolverParameters::LinearSolverParameters() {
  ...
  set_parameter("Use_trilinos_for_assembly", false, !required,
