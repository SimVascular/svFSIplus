  ...
  for (int e = 0; e < lM.nEl; e++) {
    ...
    for (int g = 0; g < fs[1].nG; g++) {
      ...
      if (nsd == 3) {
        auto N0 = fs[0].N.col(g);
        auto N1 = fs[1].N.col(g);
        ustruct_3d_c(com_mod, cep_mod, vmsStab, fs[0].eNoN, fs[1].eNoN, w, Jac, N0, N1, Nwx, Nqx, al, yl, dl, bfl, lR, lK, lKd);

      } else if (nsd == 2) {
        auto N0 = fs[0].N.col(g);
        auto N1 = fs[1].N.col(g);
        ustruct_2d_c(com_mod, cep_mod, vmsStab, fs[0].eNoN, fs[1].eNoN, w, Jac, N0, N1, Nwx, Nqx, al, yl, dl, bfl, lR, lK, lKd);
      }
...
void ustruct_2d_c(ComMod& com_mod, CepMod& cep_mod, const bool vmsFlag, const int eNoNw, const int eNoNq,
    ...
    const Array<double>& dl, const Array<double>& bfl, Array<double>& lR, Array3<double>& lK, Array3<double>& lKd)
{
  using namespace consts;
    ...
    Array3<double>& lK, Array3<double>& lKd)
{
 ...
 for (int a = 0; a < eNoNw; a++) {
  ...
  for (int b = 0; b < eNoNw; b++) {
    for (int a = 0; a < eNoNw; a++) {
      double T0, T1, T2, T3, Ku, BtDB;
      Array<double> DBm(3,2);

      double NxSNx = Nwx(0,a)*Siso(0,0)*Nwx(0,b) + Nwx(0,a)*Siso(0,1)*Nwx(1,b) + 
      ...
      T0   = Jac*rho*vd(0)*Nw(a)*NxFi(0,b);
      T1   = -tauC*Jac*NxFi(0,a)*VxNx(0,b);
      Ku   = w*af*(T0 + T1 + BtDB + NxSNx);
      lKd(0,a,b) = lKd(0,a,b) + Ku;

      T0   = am*Jac*rho*Nw(a)*Nw(b);
      T1   = af*Jac*tauC*rho*NxFi(0,a)*NxFi(0,b) + T0;
      lK(0,a,b)  = lK(0,a,b) + w*T1 + afm*Ku;

      // dM_1/dV_2 + af/am *dM_1/dU_2 
      BtDB = Bm(0,0,a)*DBm(0,1) + Bm(1,0,a)*DBm(1,1) + Bm(2,0,a)*DBm(2,1);
      T0   = Jac*rho*vd(0)*Nw(a)*NxFi(1,b);
      T1   = -tauC*Jac*NxFi(0,a)*VxNx(1,b);
      T2   = Jac*rCl*(NxFi(0,a)*NxFi(1,b) - NxFi(1,a)*NxFi(0,b));
      Ku   = w*af*(T0 + T1 + T2 + BtDB);
      lKd(1,a,b) = lKd(1,a,b) + Ku;

      T1   = af*Jac*tauC*rho*NxFi(0,a)*NxFi(1,b);
      lK(1,a,b) = lK(1,a,b) + w*T1 + afm*Ku;

      // dM_2/dV_1 + af/am *dM_2/dU_1 
      BtDB = Bm(0,1,a)*DBm(0,0) + Bm(1,1,a)*DBm(1,0) + Bm(2,1,a)*DBm(2,0);
      T0   = Jac*rho*vd(1)*Nw(a)*NxFi(0,b);
      T1   = -tauC*Jac*NxFi(1,a)*VxNx(0,b);
      T2   = Jac*rCl*(NxFi(1,a)*NxFi(0,b) - NxFi(0,a)*NxFi(1,b));
      Ku   = w*af*(T0 + T1 + T2 + BtDB);
      lKd(2,a,b) = lKd(2,a,b) + Ku;

      T1   = af*Jac*tauC*rho*NxFi(1,a)*NxFi(0,b);
      lK(3,a,b) = lK(3,a,b) + w*T1 + afm*Ku;

      // 
      BtDB = Bm(0,1,a)*DBm(0,1) + Bm(1,1,a)*DBm(1,1) + Bm(2,1,a)*DBm(2,1);
      T0   = Jac*rho*vd(1)*Nw(a)*NxFi(1,b);
      T1   = -tauC*Jac*NxFi(1,a)*VxNx(1,b);
      Ku   = w*af*(T0 + T1 + BtDB + NxSNx);
      lKd(3,a,b) = lKd(3,a,b) + Ku;

      T0   = am*Jac*rho*Nw(a)*Nw(b);
      T1   = af*Jac*tauC*rho*NxFi(1,a)*NxFi(1,b) + T0;
      lK(4,a,b) = lK(4,a,b) + w*T1 + afm*Ku;
    }
  }
    ...
    Array3<double>& lK, Array3<double>& lKd)
{
  ...
  for (int b = 0; b < eNoNw; b++) {
    for (int a = 0; a < eNoNw; a++) {
      ...
      double NxSNx = Nwx(0,a)*Siso(0,0)*Nwx(0,b) + Nwx(0,a)*Siso(0,1)*Nwx(1,b) + Nwx(0,a)*Siso(0,2)*Nwx(2,b) + 
                     Nwx(1,a)*Siso(1,0)*Nwx(0,b) + Nwx(1,a)*Siso(1,1)*Nwx(1,b) + Nwx(1,a)*Siso(1,2)*Nwx(2,b) + 
                     Nwx(2,a)*Siso(2,0)*Nwx(0,b) + Nwx(2,a)*Siso(2,1)*Nwx(1,b) + Nwx(2,a)*Siso(2,2)*Nwx(2,b);

      auto DBm = mat_fun::mat_mul(Dm, Bm.slice(b));

      // dM1_dV1 + af/am *dM_1/dU_1
      //
      BtDB = Bm(0,0,a)*DBm(0,0) + Bm(1,0,a)*DBm(1,0) + Bm(2,0,a)*DBm(2,0) + Bm(3,0,a)*DBm(3,0) +
      ...
      T2 = af * Jac * tauC * rho * NxFi(0,a) * NxFi(0,b) + T1;
      lK(0,a,b) = lK(0,a,b) + w*T2 + afm*Ku;
      ...
      T1 = af*Jac*tauC*rho*NxFi(0,a)*NxFi(2,b);
      lK(2,a,b) = lK(2,a,b) + w*T1 + afm*Ku;

      // dM_1/dV_0 + af/am *dM_1/dU_0
      //
      ...
      T0   = Jac*rho*vd(1)*Nw(a)*NxFi(2,b);
      T1   = -tauC*Jac*NxFi(1,a)*VxNx(2,b);
      T2   = Jac*rCl*(NxFi(1,a)*NxFi(2,b) - NxFi(2,a)*NxFi(1,b));
      Ku   = w*af*(T0 + T1 + T2 + BtDB);
      lKd(5,a,b) = lKd(5,a,b) + Ku;

      T1 = af*Jac*tauC*rho*NxFi(1,a)*NxFi(2,b);
      lK(6,a,b) = lK(6,a,b) + w*T1 + afm*Ku;

      // dM_2/dV_0 + af/am *dM_2/dU_0
      //
      BtDB = Bm(0,2,a)*DBm(0,0) + Bm(1,2,a)*DBm(1,0) +
             Bm(2,2,a)*DBm(2,0) + Bm(3,2,a)*DBm(3,0) +
             Bm(4,2,a)*DBm(4,0) + Bm(5,2,a)*DBm(5,0);
      T0   = Jac*rho*vd(2)*Nw(a)*NxFi(0,b);
      T1   = -tauC*Jac*NxFi(2,a)*VxNx(0,b);
      T2   = Jac*rCl*(NxFi(2,a)*NxFi(0,b) - NxFi(0,a)*NxFi(2,b));
      Ku   = w*af*(T0 + T1 + T2 + BtDB);
      lKd(6,a,b) = lKd(6,a,b) + Ku;

      T1   = af*Jac*tauC*rho*NxFi(2,a)*NxFi(0,b);
      lK(8,a,b) = lK(8,a,b) + w*T1 + afm*Ku;
      ...
      T0   = Jac*rho*vd(2)*Nw(a)*NxFi(1,b);
      T1   = -tauC*Jac*NxFi(2,a)*VxNx(1,b);
      T2   = Jac*rCl*(NxFi(2,a)*NxFi(1,b) - NxFi(1,a)*NxFi(2,b));
      Ku   = w*af*(T0 + T1 + T2 + BtDB);
      lKd(7,a,b) = lKd(7,a,b) + Ku;

      T1   = af*Jac*tauC*rho*NxFi(2,a)*NxFi(1,b);
      lK(9,a,b) = lK(9,a,b) + w*T1 + afm*Ku;
      ...
      T0   = Jac*rho*vd(2)*Nw(a)*NxFi(2,b);
      T1   = -tauC*Jac*NxFi(2,a)*VxNx(2,b);
      Ku   = w*af*(T0 + T1 + BtDB + NxSNx);
      lKd(8,a,b) = lKd(8,a,b) + Ku;

      T0   = am*Jac*rho*Nw(a)*Nw(b);
      T1   = af*Jac*tauC*rho*NxFi(2,a)*NxFi(2,b) + T0;
      lK(10,a,b) = lK(10,a,b) + w*T1 + afm*Ku;
    }
  }
