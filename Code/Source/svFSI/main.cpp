...
// The functions defined here are used to run a simulation from the command
// line.
//
// Usage:
//
...
void read_files(Simulation* simulation, const std::string& file_name) {
  ...
  /*
    try {
      ...
      std::cout << "[svFSIplus] ERROR The svFSIplus program has failed." <<
    std::endl; std::cout << "[svFSIplus] ERROR " << exception.what() <<
    std::endl; exit(1);
    }
  */
}
...
//
      ...
      for (int iM = 0; iM < com_mod.nMsh; iM++) {
        eq_assem::global_eq_assem(com_mod, cep_mod, com_mod.msh[iM], Ag, Yg,
                                  Dg);
      }
...
//
// Modifies: com_mod.An com_mod.Dn com_mod.Yn cep_mod.Xion com_mod.pS0
// com_mod.pSa
...
//
      ...
      if (std::count_if(com_mod.eq.begin(), com_mod.eq.end(), [](eqType& eq) {
            return eq.ok;
          }) == com_mod.eq.size()) {
#ifdef debug_iterate_solution
...
#ifdef debug_iterate_solution
    dmsg << "Saving the TXT files containing average and fluxes ..."
         << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  // Create a Simulation object that stores all data structures for a
  // simulation.
  //
