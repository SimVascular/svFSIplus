...
//
using GetElementGausIntMapType =
...
GetElementGausIntMapType get_element_gauss_int_data = {
    ...
    {ElementType::HEX8,
     [](const int insd, const int nG, Vector<double>& w,
        Array<double>& xi) -> void {
       w = 1.0;
...
//
using SetFaceGausIntMapType =
    std::map<ElementType, std::function<void(faceType&)>>;
