...
void cmm_3d(ComMod& com_mod, const int eNoN, const double w,
...
#define n_debug_cmm_3d
  ...
  for (int a = 0; a < eNoN; a++) {
    es_x(0, a) =
        es(0, 0) * Nx(0, a) + es(1, 0) * Nx(1, a) + es(2, 0) * Nx(2, a);
    es_x(1, a) =
        es(0, 1) * Nx(0, a) + es(1, 1) * Nx(1, a) + es(2, 1) * Nx(2, a);
    es_x(2, a) =
        es(0, 2) * Nx(0, a) + es(1, 2) * Nx(1, a) + es(2, 2) * Nx(2, a);
  }
  ...
  for (int a = 0; a < eNoN; a++) {
    ...
    lR(0, a) =
        lR(0, a) + wr * N(a) * rV(0) +
        w * (Nx(0, a) * rM(0, 0) + Nx(1, a) * rM(1, 0) + Nx(2, a) * rM(2, 0));
    lR(1, a) =
        lR(1, a) + wr * N(a) * rV(1) +
        w * (Nx(0, a) * rM(0, 1) + Nx(1, a) * rM(1, 1) + Nx(2, a) * rM(2, 1));
    lR(2, a) =
        lR(2, a) + wr * N(a) * rV(2) +
        w * (Nx(0, a) * rM(0, 2) + Nx(1, a) * rM(1, 2) + Nx(2, a) * rM(2, 2));
  ...
  for (int a = 0; a < eNoN; a++) {
    for (int b = 0; b < eNoN; b++) {
      ...
      double NxNx =
          Nx(0, a) * Nx(0, b) + Nx(1, a) * Nx(1, b) + Nx(2, a) * Nx(2, b);
      double T1 = mu * NxNx + tauB * upNx(a) * upNx(b) +
                  rho * (N(a) * (amd * N(b) + uaNx(b)) +
      ...
      // dM/dU
      //
      lK(0, a, b) = lK(0, a, b) + wl * ((mu + tauC) * rM(0, 0) + T1 +
                                        mu_x * es_x(0, a) * es_x(0, b));
      lK(1, a, b) = lK(1, a, b) + wl * (mu * rM(1, 0) + tauC * rM(0, 1) +
                                        mu_x * es_x(0, a) * es_x(1, b));
      lK(2, a, b) = lK(2, a, b) + wl * (mu * rM(2, 0) + tauC * rM(0, 2) +
                                        mu_x * es_x(0, a) * es_x(2, b));
      lK(4, a, b) = lK(4, a, b) + wl * (mu * rM(0, 1) + tauC * rM(1, 0) +
                                        mu_x * es_x(1, a) * es_x(0, b));
      lK(5, a, b) = lK(5, a, b) + wl * ((mu + tauC) * rM(1, 1) + T1 +
                                        mu_x * es_x(1, a) * es_x(1, b));
      lK(6, a, b) = lK(6, a, b) + wl * (mu * rM(2, 1) + tauC * rM(1, 2) +
                                        mu_x * es_x(1, a) * es_x(2, b));
      lK(8, a, b) = lK(8, a, b) + wl * (mu * rM(0, 2) + tauC * rM(2, 0) +
                                        mu_x * es_x(2, a) * es_x(0, b));
      lK(9, a, b) = lK(9, a, b) + wl * (mu * rM(1, 2) + tauC * rM(2, 1) +
                                        mu_x * es_x(2, a) * es_x(1, b));
      lK(10, a, b) = lK(10, a, b) + wl * ((mu + tauC) * rM(2, 2) + T1 +
                                          mu_x * es_x(2, a) * es_x(2, b));
...
void cmm_b(ComMod& com_mod, const faceType& lFa, const int e,
...
#ifdef WITH_TRILINOS
    int n = 3;
    trilinos_doassem_(const_cast<int&>(n), const_cast<int*>(ptr.data()),
                      lK.data(), lR.data());
#endif
  } else {
...
#define n_debug_cmmi
  ...
  if (eq.assmTLS) {
    ...
    trilinos_doassem_(numNodesPerElement, const_cast<int*>(ptr.data()),
                      lK.data(), lR.data());
  } else {
#endif
    lhsa_ns::do_assem(com_mod, 3, ptr, lK, lR);
...
#define n_debug_cmm_stiffness
  ...
  for (int a = 0; a < 3; a++) {
    xloc(0, a) =
        thet(0, 0) * xl(0, a) + thet(0, 1) * xl(1, a) + thet(0, 2) * xl(2, a);
    xloc(1, a) =
        thet(1, 0) * xl(0, a) + thet(1, 1) * xl(1, a) + thet(1, 2) * xl(2, a);
...
#define n_debug_construct_cmm
  ...
  Array<double> xl(nsd, eNoN), al(tDof, eNoN), yl(tDof, eNoN), dl(tDof, eNoN),
      vwpl(2, eNoN), bfl(nsd, eNoN), pS0l(nsymd, eNoN), Nx(nsd, eNoN),
      lR(dof, eNoN);
    ...
    if (cmmInit) {
      if (lM.eType != ElementType::TRI3) {
        throw std::runtime_error(
            "[construct_cmm] CMM initialization is allowed for triangular "
            "meshes only");
      }
    } else {
      if (lM.eType != ElementType::TET4) {
        throw std::runtime_error(
            "[construct_cmm] CMM equation is allowed for tetrahedral meshes "
            "only");
      }
    }
      ...
      for (int g = 0; g < lM.nG; g++) {
        if (g == 0 || !lM.lShpF) {
          ...
          if (utils::is_zero(Jac)) {
            throw std::runtime_error(
                "[construct_dsolid] Jacobian for element " + std::to_string(e) +
                " is < 0.");
          }
        }
        double w = lM.w(g) * Jac;
