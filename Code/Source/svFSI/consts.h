...
enum class BoundaryConditionType {
  ...
  bType_impD =
      12,  // impose BC on the integral of state variable or D (instead of Y)
...
// Define constants using smaller name and integer value (needed for bitwise
// operations).
//
...
extern const std::map<std::string, ConstitutiveModelType>
...
enum class EquationType {
  ...
  phys_FSI = 206,
  phys_mesh =
      207,  // solves a modified lElas for mesh motion; should be used with FSI
  phys_shell =
      208,  // solves nonlinear thin shell mechanics (Kirchhoff-Love theory)
  phys_CMM = 209,
  phys_CEP = 210,
  phys_ustruct =
      211,  // Nonlinear elastodynamics using mixed VMS-stabilized formulation
  phys_stokes = 212
};
...
extern const std::map<std::string, MeshGeneratorType>
...
/// Map for preconditioner type string to pair (PreconditionerType enum,
/// bool(true if Trilinos precondition)).
using PreconditionerMapType = std::pair<PreconditionerType, bool>;
// extern const std::map<std::string,PreconditionerType>
// preconditioner_name_to_type;
extern const std::map<std::string, PreconditionerMapType>
    preconditioner_name_to_type;
...
extern const std::map<std::string, FluidViscosityModelType>
