...
#define n_debug_get_pk2cc
  ...
  switch (stM.isoType) {
    ...
    case ConstitutiveModelType::stIso_nHook: {
      ...
      S += p * J * Ci;
      CC += 2.0 * (r1 - p * J) * ten_symm_prod(Ci, Ci, nsd) +
            (pl * J - 2.0 * r1 / nd) * ten_dyad_prod(Ci, Ci, nsd);
    ...
    case ConstitutiveModelType::stIso_MR: {
      ...
      S = S + p * J * Ci;
      CC = CC + 2.0 * (r1 - p * J) * ten_symm_prod(Ci, Ci, nsd) +
           (pl * J - 2.0 * r1 / nd) * ten_dyad_prod(Ci, Ci, nsd);
    } break;
    ...
    case ConstitutiveModelType::stIso_HGO: {
      if (nfd != 2) {
        throw std::runtime_error(
            "[get_pk2cc] Min fiber directions not defined for HGO material "
            "model.");
      }
      double kap = stM.kap;
      ...
      g1 = stM.aff * (1.0 + 2.0 * stM.bff * Eff * Eff) *
           exp(stM.bff * Eff * Eff);
      g2 = stM.ass * (1.0 + 2.0 * stM.bss * Ess * Ess) *
           exp(stM.bss * Ess * Ess);
      g1 = 4.0 * J4d * g1;
      g2 = 4.0 * J4d * g2;

      auto CCb =
          g1 * ten_dyad_prod(Hff, Hff, nsd) + g2 * ten_dyad_prod(Hss, Hss, nsd);
      double r1 = J2d * mat_ddot(C, Sb, nsd) / nd;
      S = J2d * Sb - r1 * Ci;
    ...
    case ConstitutiveModelType::stIso_Gucci: {
      if (nfd != 2) {
        throw std::runtime_error(
            "[get_pk2cc] Min fiber directions not defined for Guccione "
            "material model.");
      }
      ...
      double QQ = g1 * Es(0, 0) * Es(0, 0) +
                  g2 * (Es(1, 1) * Es(1, 1) + Es(2, 2) * Es(2, 2) +
                        Es(1, 2) * Es(1, 2) + Es(2, 1) * Es(2, 1)) +
                  g3 * (Es(0, 1) * Es(0, 1) + Es(1, 0) * Es(1, 0) +
                        Es(0, 2) * Es(0, 2) + Es(2, 0) * Es(2, 0));
      ...
      auto Sb =
          g1 * Es(0, 0) * RmRm.slice(0) +
          g2 * (Es(1, 1) * RmRm.slice(1) + Es(2, 2) * RmRm.slice(2) +
                2.0 * Es(1, 2) * RmRm.slice(4)) +
      ...
      CCb =
          r2 * (CCb + g1 * ten_dyad_prod(RmRm.slice(0), RmRm.slice(0), nsd) +
                ...
                2.0 * g3 *
                    (ten_dyad_prod(RmRm.slice(3), RmRm.slice(3), nsd) +
    ...
    case ConstitutiveModelType::stIso_HO: {
      if (nfd != 2) {
        throw std::runtime_error(
            "[get_pk2cc] Min fiber directions not defined for Holzapfel "
            "material model.");
      }
      double Inv4 = J2d * utils::norm(fl.col(0), mat_mul(C, fl.col(0)));
      double Inv6 = J2d * utils::norm(fl.col(1), mat_mul(C, fl.col(1)));
      ...
      Efs = Efs * Efs;
      g1 = 2.0 * J4d * stM.b * g1;
      g2 = 4.0 * J4d * stM.afs * (1.0 + 2.0 * stM.bfs * Efs) *
           exp(stM.bfs * Efs);
      auto CCb =
          g1 * ten_dyad_prod(Idm, Idm, nsd) + g2 * ten_dyad_prod(Hfs, Hfs, nsd);
      ...
      if (Eff > 0.0) {
        ...
        Eff = Eff * Eff;
        g1 = 4.0 * J4d * stM.aff * (1.0 + 2.0 * stM.bff * Eff) *
             exp(stM.bff * Eff);
      ...
      if (Ess > 0.0) {
        ...
        Ess = Ess * Ess;
        g2 = 4.0 * J4d * stM.ass * (1.0 + 2.0 * stM.bss * Ess) *
             exp(stM.bss * Ess);
...
#define n_debug_get_pk2cc_dev
  ...
  switch (stM.isoType) {
    ...
    case ConstitutiveModelType::stIso_HGO: {
      if (nfd != 2) {
        throw std::runtime_error(
            "[get_pk2cc_dev] Min fiber directions not defined for HGO material "
            "model.");
      }
    ...
    case ConstitutiveModelType::stIso_Gucci: {
      if (nfd != 2) {
        throw std::runtime_error(
            "[get_pk2cc_dev] Min fiber directions not defined for Guccione "
            "material model.");
      }
    ...
    case ConstitutiveModelType::stIso_HO: {
      if (nfd != 2) {
        throw std::runtime_error(
            "[get_pk2cc_dev] Min fiber directions not defined for Holzapfel "
            "material model.");
      }
    ...
    default:
      throw std::runtime_error(
          "Undefined isochoric material constitutive model.");
  }
...
#define n_debug_get_pk2cc_shlc
  ...
  while (true) {
    ...
    // Trace (C)
    auto trC3 = (gg_x(0, 0) * gi_0(0, 0) + gg_x(0, 1) * gi_0(0, 1) +
                 gg_x(1, 0) * gi_0(1, 0) + gg_x(1, 1) * gi_0(1, 1) + C33) *
                f13;
    ...
    switch (stM.isoType) {
      case ConstitutiveModelType::stIso_nHook: {
        ...
        // Elasticity tensor
        CC = (mu * J23 * f23 * trC3 + plJ) * ten_dyad_prod(Ci, Ci, 3) +
             (mu * J23 * trC3 - pJ) * 2.0 * ten_symm_prod(Ci, Ci, 3) -
             f23 * mu * J23 *
                 (ten_dyad_prod(gi_0, Ci, 3) + ten_dyad_prod(Ci, gi_0, 3));
      } break;
      ...
      case ConstitutiveModelType::stIso_MR: {
        ...
        auto I2ijkl =
            ten_dyad_prod(gi_0, gi_0, 3) - ten_symm_prod(gi_0, gi_0, 3);
        ...
        S = C1 * J23 * (gi_0 - trC3 * Ci) + pJ * Ci +
            C2 * J43 * (I2ij - f23 * I2 * Ci);

        //  Elasticity tensor
        CC = (C1 * J23 * f23 * trC3 + plJ) * ten_dyad_prod(Ci, Ci, 3) +
             (C1 * J23 * trC3 - pJ) * 2.0 * ten_symm_prod(Ci, Ci, 3) -
             f23 * C1 * J23 *
                 (ten_dyad_prod(gi_0, Ci, 3) + ten_dyad_prod(Ci, gi_0, 3));

        CC += 2.0 * f23 * C2 * J43 *
              (ten_dyad_prod((f23 * I2 * Ci - I2ij), Ci, 3) - I2 * Cikl -
      ...
      case ConstitutiveModelType::stIso_HO_ma: {
        if (nfd != 2) {
          throw std::runtime_error(
              "Min fiber directions not defined for Holzapfel material model "
              "(1)");
        }
        ...
        //
        auto Inv4 = gg_x(0, 0) * fl(0, 0, 0) + gg_x(0, 1) * fl(0, 1, 0) +
                    gg_x(1, 0) * fl(1, 0, 0) + gg_x(1, 1) * fl(1, 1, 0);
        auto Inv6 = gg_x(0, 0) * fl(0, 0, 1) + gg_x(0, 1) * fl(0, 1, 1) +
                    gg_x(1, 0) * fl(1, 0, 1) + gg_x(1, 1) * fl(1, 1, 1);
        auto Inv8 = gg_x(0, 0) * fNa0(0, 0) * fNa0(0, 1) +
                    gg_x(0, 1) * fNa0(0, 0) * fNa0(1, 1) +
                    gg_x(1, 0) * fNa0(1, 0) * fNa0(0, 1) +
                    gg_x(1, 1) * fNa0(1, 0) * fNa0(1, 1);
        ...
        CC = d1 * CC + plJ * ten_dyad_prod(Ci, Ci, 3) -
             pJ * 2.0 * ten_symm_prod(Ci, Ci, 3);
      ...
      case ConstitutiveModelType::stIso_LS: {
        ...
        //
        auto d1 = 2.0 * stM.a0 * stM.mu0 * stM.b1 * J23 *
                  exp(2.0 * stM.b1 * (trC3 * J23 - 1.0));
        auto SN = (gi_0 - trC3 * Ci);
        auto CCN =
            f23 * trC3 * ten_dyad_prod(Ci, Ci, 3) +
            trC3 * 2.0 * ten_symm_prod(Ci, Ci, 3) -
            f23 * (ten_dyad_prod(gi_0, Ci, 3) + ten_dyad_prod(Ci, gi_0, 3));

        S = (stM.a + d1 * (2.0 * (trC3 * J23 - 1.0))) * SN + pJ * Ci;

        CC = (1.0 + 18.0 * stM.b1 * (trC3 * J23 - 1.0) * (trC3 * J23 - 1.0)) *
                 J23 * ten_dyad_prod(SN, SN, 3) +
             3.0 * (trC3 * J23 - 1.0) * CCN;
        CC = stM.a * CCN + 2.0 * d1 * CC + plJ * ten_dyad_prod(Ci, Ci, 3) -
             pJ * 2.0 * ten_symm_prod(Ci, Ci, 3);
        ...
        if (Eff > 0.0) {
          ...
          S += 2.0 * stM.a0 * (1.0 - stM.mu0) * stM.b2 * Eff *
               exp(stM.b2 * Eff * Eff) * flM;

          CC += 4.0 * stM.a0 * (1.0 - stM.mu0) * stM.b2 *
                (1.0 + 2.0 * stM.b2 * Eff * Eff) * exp(stM.b2 * Eff * Eff) *
                ten_dyad_prod(flM, flM, 3);
        }
    ...
    if (itr > MAXITR) {
      std::cerr << "[get_pk2cc_shlc] Failed to converge plane-stress condition."
                << std::endl;
      // exit(0);
      break;
    }
...
#define n_debug_get_pk2cc_shli
  ...
  switch (stM.isoType) {
    case ConstitutiveModelType::stIso_nHook: {
      ...
      S = mu * (gi_0 - Jg2i * gi_x);
      auto CC = 2.0 * mu * Jg2i *
                (ten_dyad_prod(gi_x, gi_x, 1) + ten_symm_prod(gi_x, gi_x, 1));
    } break;
    ...
    case ConstitutiveModelType::stIso_MR: {
      ...
      CC = (stM.C10 + stM.C01 * I1) * CCN -
           2.0 * stM.C01 * Jg2i *
               (ten_dyad_prod(gi_0, gi_x, 1) + ten_dyad_prod(gi_x, gi_0, 1)) +
           2.0 * stM.C01 / Jg2i *
               (ten_dyad_prod(gi_x, gi_0, 1) - ten_symm_prod(gi_x, gi_x, 1));
    } break;
    ...
    case ConstitutiveModelType::stIso_LS: {
      ...
      auto EI1 = (I1 + Jg2i - 3.0);
      S = stM.a * SN +
          2.0 * stM.a0 * stM.mu0 * stM.b1 * EI1 * exp(stM.b1 * EI1 * EI1) * SN;
      CC = stM.a * CCN +
           4.0 * stM.a0 * stM.mu0 * stM.b1 * exp(stM.b1 * EI1 * EI1) *
               ((1.0 + 2.0 * stM.b1 * EI1 * EI1) * ten_dyad_prod(SN, SN, 1) +
                0.50 * EI1 * CCN);
