...
#define n_debug_get_pk2cc
  ...
  msg_prefix =
      std::string("[get_pk2cc(fixed):") + std::to_string(task_id) + "] ";
#ifdef debug_get_pk2cc
  std::cout << msg_prefix << std::endl;
  std::cout << msg_prefix
            << "========== get_pk2cc fixed ==========" << std::endl;
#endif
  ...
  */
  ...
  double Ci[N][N];
  mat_inv<N>(C, Ci);
  // std::cout << msg_prefix << "num_allocated 2: " <<
  // Array<double>::num_allocated - num_alloc << std::endl;
  ...
  switch (stM.isoType) {
    ...
    case ConstitutiveModelType::stIso_mStVK: {
      ...
      // CC = g1 * ( -2.0*log(J)*ten_symm_prod(Ci, Ci, nsd) + ten_dyad_prod(Ci,
      // Ci, nsd) ) + 2.0*g2*ten_ids(nsd);
    } break;
      ...
      for (int i = 0; i < nsd; i++) {
        for (int j = 0; j < nsd; j++) {
          for (int k = 0; k < nsd; k++) {
            for (int l = 0; l < nsd; l++) {
              CC[i][j][k][l] =
                  (-2.0 / nsd) * Ci_S_prod[i][j][k][l] + S_Ci_prod[i][j][k][l];
            }
          }
        }
      }
      // CC = (-2.0/nd) * ( ten_dyad_prod(Ci, S, nsd) + ten_dyad_prod(S, Ci,
      // nsd));
      ...
      for (int i = 0; i < nsd; i++) {
        for (int j = 0; j < nsd; j++) {
          for (int k = 0; k < nsd; k++) {
            for (int l = 0; l < nsd; l++) {
              CC[i][j][k][l] +=
                  2.0 * (r1 - p * J) * Ci_sym_prod[i][j][k][l] +
                  (pl * J - 2.0 * r1 / nd) * Ci_Ci_prod[i][j][k][l];
            }
          }
        }
      }
      // CC += 2.0*(r1 - p*J) * ten_symm_prod(Ci, Ci, nsd)  +  (pl*J
      // - 2.0*r1/nd) * ten_dyad_prod(Ci, Ci, nsd);
    ...
    case ConstitutiveModelType::stIso_MR: {
      ...
      for (int i = 0; i < nsd; i++) {
        for (int j = 0; j < nsd; j++) {
          for (int k = 0; k < nsd; k++) {
            for (int l = 0; l < nsd; l++) {
              PP[i][j][k][l] =
                  Ids[i][j][k][l] - (1.0 / nd) * Ci_C_prod[i][j][k][l];
            }
          }
        }
      }
      ...
      for (int i = 0; i < nsd; i++) {
        for (int j = 0; j < nsd; j++) {
          for (int k = 0; k < nsd; k++) {
            for (int l = 0; l < nsd; l++) {
              CC[i][j][k][l] -=
                  (2.0 / nd) * (Ci_S_prod[i][j][k][l] + S_Ci_prod[i][j][k][l]);
            }
          }
        }
      }
      // CC = CC - (2.0/nd) * ( ten_dyad_prod(Ci, S, nsd) + ten_dyad_prod(S, Ci,
      // nsd) );
      ...
      // CC = CC + 2.0*(r1 - p*J) * ten_symm_prod(Ci, Ci, nsd) + (pl*J
      // - 2.0*r1/nd) * ten_dyad_prod(Ci, Ci, nsd);
    ...
    case ConstitutiveModelType::stIso_HGO: {
      ...
      /*
            ...
            auto CCb = g1 * ten_dyad_prod(Hff, Hff, nsd) + g2 *
         ten_dyad_prod(Hss, Hss, nsd); double r1  = J2d*mat_ddot(C, Sb, nsd) /
         nd; S = J2d*Sb - r1*Ci;
            ...
            CC = ten_transpose(CC, nsd);
            CC = ten_ddot(PP, CC, nsd);
            CC = CC - (2.0/nd) * ( ten_dyad_prod(Ci, S, nsd) + ten_dyad_prod(S,
         Ci, nsd) );
    ...
    case ConstitutiveModelType::stIso_Gucci: {
      /*
            ...
            #endif
            if (nfd != 2) {
              //err = "Min fiber directions not defined for Guccione material
      model (2)"
            }
      ...
      Es(1,2)*Es(1,2) + Es(2,1)*Es(2,1)) + g3 * (Es(0,1)*Es(0,1) +
      Es(1,0)*Es(1,0) + Es(0,2)*Es(0,2) + Es(2,0)*Es(2,0));
      ...
      + 2.0*Es(1,2)*RmRm.slice(4)) + 2.0 * g3 * (Es(0,1) * RmRm.slice(3) +
      Es(0,2)*RmRm.slice(5));
            ...
            CCb = r2*(CCb + g1 * ten_dyad_prod(RmRm.slice(0), RmRm.slice(0),
      nsd) + g2 * (ten_dyad_prod(RmRm.slice(1), RmRm.slice(1), nsd) +
                                 ten_dyad_prod(RmRm.slice(2), RmRm.slice(2),
      nsd) + ten_dyad_prod(RmRm.slice(4), RmRm.slice(4), nsd)*2.0) + 2.0 * g3 *
      (ten_dyad_prod(RmRm.slice(3), RmRm.slice(3), nsd) +
            ...
            for (int i = 0; i < nsd; i++) {
              for (int j = 0; j < nsd; j++) {
                for (int k = 0; k < nsd; k++) {
                  for (int l = 0; l < nsd; l++) {
                    std::cout << msg_prefix << "CCb: " << i+1 << " " << j+1 << "
      " << k+1 << " " << l+1 << " " << CCb(i,j,k,l) << std::endl;
                  }
                }
              }
            }
    ...
    case ConstitutiveModelType::stIso_HO: {
      if (nfd != 2) {
        // err = "Min fiber directions not defined for Holzapfel material model
        // (2)"
      }
      /*
            double Inv4 = J2d*utils::norm(fl.col(0), mat_mul(C, fl.col(0)));
            ...
            auto CCb  = g1 * ten_dyad_prod(Idm, Idm, nsd) + g2 *
         ten_dyad_prod(Hfs, Hfs, nsd);
