...
/// @brief The Tensor4 template class implements a simple interface to 4th order
/// tensors.
//
template <typename T>
class Tensor4
{
  public:
    ...
    Tensor4& operator=(const Tensor4& rhs)
    {
      ...
      if (ni_ != rhs.ni_ || nj_ != rhs.nj_ || nk_ != rhs.nk_ ||
          nl_ != rhs.nl_ <= 0) {
        clear();
        // delete [] data_;
        // data_ = nullptr;
    ...
    //----------
    //
    void allocate(const int num_i, const int num_j, const int num_k,
                  const int num_l)
    {
    ...
    void check_index(const int i, const int j, const int k, const int l) const
    {
      ...
      if ((i < 0) || (i >= ni_) || (j < 0) || (j >= nj_) || (k < 0) ||
        ...
        throw std::runtime_error("Index (i,j,k,l)=" + index_str +
    ...
    // Resize the array.
    //
    void resize(const int num_i, const int num_j, const int num_k,
                const int num_l)
    {
      if (data_ != nullptr) {
    ...
    const T& operator()(const int i, const int j, const int k,
                        const int l) const
    {
