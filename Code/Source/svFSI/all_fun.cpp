...
void commu(const ComMod& com_mod, Vector<double>& U) {
  ...
  if (U.size() != com_mod.lhs.nNo) {
    throw std::runtime_error(
        "COMMU is only specified for vector with size nNo");
  }
...
void find_face(const std::vector<mshType>& mesh_list,
  ...
  if (iM == -1) {
    throw std::runtime_error("Can't find face named '" + faceName +
                             "' from defined mesh names.");
  }
}
...
#define n_debug_global_rv
  ...
  if (U.ncols() != lM.nNo) {
    throw std::runtime_error(
        "GLOBAL is only specified for array with columns size nNo");
  }
  ...
  MPI_Gatherv(ienU.data(), lM.nEl * a, cm_mod::mpreal, gienU.data(),
...
#define n_debug_integ_v
  ...
  if (l != u) {
    throw std::runtime_error(
        "Incompatible spatial output setting and element type");
  }
      ...
      for (int e = 0; e < msh.nEl; e++) {
        ...
        for (int g = 0; g < fs.nG; g++) {
          ...
          if (utils::is_zero(Jac)) {
            throw std::runtime_error(
                "Jac < 0 for element: " + std::to_string(e) + ")");
          }
          double sHat = 0.0;
...
#define n_debug_integ_s
  ...
  if (flag) {
    if (lFa.nFs != 2) {
      throw std::runtime_error(
          "Incompatible boundary integral function call and face element type");
    }
...
///
/// Note that 'l' seems to be a length and 'uo' an offset. 'l' should never be
/// 0.
///
...
bool is_domain(const ComMod& com_mod, const eqType& eq, const int node,
  ...
  if (eq.nDmn == 1) {
    ...
    //  IF (.NOT.ALLOCATED(dmnId)) err = "Domain partitioning info "// 2 "is not
    //  provided"
...
Vector<int> local(const ComMod& com_mod, const CmMod& cm_mod, const cmType& cm,
  ...
  if (cm.mas(cm_mod)) {
    if (U.size() != com_mod.gtnNo) {
      throw std::runtime_error(
          "local_is is only specified for vector with size gtnNo");
    }
  }
...
Array<double> local(const ComMod& com_mod, const CmMod& cm_mod,
  ...
  if (cm.mas(cm_mod)) {
    ...
    if (U.ncols() != com_mod.gtnNo) {
      throw std::runtime_error(
          "local_rv is only specified for vector with size gtnNo");
    }
  }
...
void set_dmn_id(mshType& mesh, const int iDmn, const int ifirst,
  ...
  if ((first < 0) || (first > mesh.gnEl - 1)) {
    throw std::runtime_error("Setting domain ID with a start element range " +
                             std::to_string(first) + ".");
  }
  ...
  if ((last < first) || (last > mesh.gnEl - 1)) {
    throw std::runtime_error("Setting domain ID with a end element range " +
                             std::to_string(last) + ".");
  }
...
double skewness(ComMod& com_mod, const int nDim, const int eNoN,
  ...
  circumRad =
      sqrt(circumRad - 4.0 * detD(0) * detD(nDim + 1)) / (2.0 * fabs(detD(0)));
