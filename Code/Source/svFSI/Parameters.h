...
/// int and string.
///
/// The classes defined here are used to process svFSIplus simulation parameters
/// read in from an Extensible Markup Language (XML) format file. XML is a
/// simple text-based format for representing structured information.
///
/// An XML document is formed as an element tree. The XML tree starts at a root
/// element and branches from the root to sub-elements. All elements can have
/// sub-elements:
///
/// \code{.cpp}
/// <svFSIFile>
...
/// Parameters class
///-----------------
/// The Parameters class is the top level class. It contains objects used to
/// store parameters for the sections making up an XML simulation parameters
/// file
///
...
///
/// Each object contains methods to parse the XML file for the parameters
/// defined for it. These section objects may also contain objects representing
/// the sub-sections defined for each section.
///
///-----------------
/// Section objects
///-----------------
/// Each section object contains objects representing parameters. A parameter's
/// name and value is stored using either a Parameter and VectorParamater
/// template objects. A parameter value is stored as a basic type: bool, double,
/// int and string.
///
/// Parameter objects in a section class are named using the same XML element
/// name with the 1st character lower case.
///
...
///   <Spectral_radius_of_infinite_time_step> Parameter<double> time_step_size;
///   // <Time_step_size>
/// \endcode
///
...
/// to tinyxml2 to parse parameter values from an XML file.
///
/// Each section object inherits from the ParameterLists class. This class
/// provides methods to store parameters in a map and process iterated over them
/// for setting values and other operations.
class Parameter
{
  public:
    Parameter(){};

    Parameter(const std::string& name, T value, bool required,
              std::vector<T> range = {})
    ...
    void set(const std::string& str_value)
    {
      ...
      if (!(str_stream >> value_)) {
        ...
        if (!(str_stream >> std::boolalpha >> value_)) {
          throw std::runtime_error("Incorrect value '" + str + "' for '" +
                                   name_ + "'.");
        }
      }
...
class VectorParameter
{
  public:
    ...
    VectorParameter(const std::string& name, const std::vector<T>& value,
    ...
    /// @brief Set the parameter name and value, and if it is required.
    void set(const std::string& name, bool required,
    ...
    void set(const std::string& str_value)
    {
      ...
      std::string error_msg = "Improper vector format '" + str_value +
                              "' found in '" + name_ + "'." +
...
class ParameterLists
{
  public:
    ...
    void set_parameter(const std::string& name, const bool value, bool required,
    ...
    void set_parameter(const std::string& name, const double value,
                       bool required, Parameter<double>& param)
    {
      param.set(name, required, value);
      params_map[name] = &param;
    }
    ...
    void set_parameter(const std::string& name,
                       std::initializer_list<double> value, bool required,
                       VectorParameter<double>& param)
    {
      param.set(name, required, value);
      params_map[name] = &param;
    }
    ...
    void set_parameter(const std::string& name,
                       std::initializer_list<int> value, bool required,
                       VectorParameter<int>& param)
    {
      param.set(name, required, value);
      params_map[name] = &param;
    }
    ...
    void set_parameter(const std::string& name,
                       std::initializer_list<std::string> value, bool required,
    ...
    void set_parameter(const std::string& name, const int value, bool required,
                       Parameter<int>& param, std::vector<int> range = {})
    {
      param.set(name, required, value);
      params_map[name] = &param;
    }
    ...
    void set_parameter(const std::string& name, const std::string& value,
                       bool required, Parameter<std::string>& param)
    {
      param.set(name, required, value);
      params_map[name] = &param;
    }
    ...
    void check_required()
    {
      ...
      for (auto& [key, param] : params_map) {
        if (std::visit([](auto&& p) { return !p->check_required_set(); },
                       param)) {
          throw std::runtime_error(xml_element_name + " XML element '" + key +
                                   "' has not been set.");
        }
      }
    }
    ...
    std::map<std::string, std::string> get_parameter_list()
    {
      ...
      for (auto& [key, param] : params_map) {
        std::visit([&params](auto&& p) { params[p->name()] = p->svalue(); },
                   param);
      }
    ...
    /// @brief Map used for storing parameters by name / Parameter template
    /// union.
    std::map<std::string,
             std::variant<Parameter<bool>*, Parameter<double>*, Parameter<int>*,
                          Parameter<std::string>*, VectorParameter<double>*,
                          VectorParameter<int>*, VectorParameter<std::string>*>>
        params_map;
...
///   <Temporal_and_spatial_values_file_path> bforce.dat
...
class RemesherParameters : public ParameterLists
{
  public:
    ...
    void print_parameters();
    double get_edge_size(const std::string& name) const
    {
      return max_edge_sizes_.at(name);
    }
    bool has_edge_size(const std::string& name) const
    {
...
///   </Searched_file_name_to_trigger_stop> <Save_results_to_VTK_format> true
///   </Save_results_to_VTK_format> <Name_prefix_of_saved_VTK_files> result
///   </Name_prefix_of_saved_VTK_files> <Increment_in_saving_VTK_files> 1
///   </Increment_in_saving_VTK_files> <Start_saving_after_time_step> 1
///   </Start_saving_after_time_step> <Increment_in_saving_restart_files> 1
///   </Increment_in_saving_restart_files> <Convert_BIN_to_VTK_format> 0
///   </Convert_BIN_to_VTK_format> <Verbose> 1 </Verbose> <Warning> 0 </Warning>
///   <Debug> 0 </Debug>
...
///       <Face_file_path> mesh/lumen/mesh-surfaces/lumen_inlet.vtp
///       </Face_file_path>
///   </Add_face>
///
///   <Add_face name="lumen_outlet">
///       <Face_file_path> mesh/lumen/mesh-surfaces/lumen_outlet.vtp
///       </Face_file_path>
///   </Add_face>
///
///   <Add_face name="lumen_wall">
///       <Face_file_path> mesh/lumen/mesh-surfaces/lumen_wall.vtp
///       </Face_file_path>
///   </Add_face>
///
///   <Domain> 0 </Domain>
...
/// @brief The Parameters class stores parameter values read in from a solver
/// input file.
class Parameters
{
  public:
