    ...
    for (int g = 0; g < lM.nG; g++) {
      if (g == 0 || !lM.lShpF) {
        ...
        if (utils::is_zero(Jac)) {
          throw std::runtime_error("[construct_heats] Jacobian for element " +
...
#define n_debug_heats_2d
  ...
  for (int a = 0; a < eNoN; a++) {
    lR(0, a) =
        lR(0, a) + w * (N(a) * Td + (Nx(0, a) * Tx(0) + Nx(1, a) * Tx(1)) * nu);

    for (int b = 0; b < eNoN; b++) {
      lK(0, a, b) =
          lK(0, a, b) + wl * (N(a) * N(b) * amd +
                              nu * (Nx(0, a) * Nx(0, b) + Nx(1, a) * Nx(1, b)));
    }
  }
}
