  ...
  for (int iEq = 0; iEq < com_mod.nEq; iEq++) {
    ...
    for (int iBc = 0; iBc < eq.nBc; iBc++) {
      ...
      if (com_mod.msh[iM].lShl) {
        shl_bc_ini(com_mod, cm_mod, bc, com_mod.msh[iM].fa[iFa],
                   com_mod.msh[iM]);
      }
    }
  }
...
#define n_debug_bc_ini
  ...
  if (btest(lBc.bType, enum_int(BoundaryConditionType::bType_gen))) {
    if (btest(lBc.bType, enum_int(BoundaryConditionType::bType_Neu)) &&
        lBc.gm.dof != 1) {
      throw std::runtime_error(
          "Only for (int F=1 is accepted for Neu general BCs");
    }
    return;
  }
  ...
  if (btest(lBc.bType, enum_int(BoundaryConditionType::bType_Robin)) &&
      not com_mod.dFlag) {
    throw std::runtime_error(
        "Robin BC can be set for a displacement-based eqn only");
  }
...
// if (.NOT.ALLOCATED(lBc.gx)) ALLOCATE(lBc.gx(lFa.nNo))
  ...
  if (btest(lBc.bType, iBC_flat)) {
    ...
    for (int i = 0; i < nsd; i++) {
      center(i) =
          all_fun::integ(com_mod, cm_mod, lFa, com_mod.x, i + 1) / lFa.area;
    }
    ...
    if (!cm.seq()) {
      int i = j * nsd;
      MPI_Allgather(&i, 1, cm_mod::mpint, sCount.data(), 1, cm_mod::mpint,
                    cm.com());
      disp(0) = 0;
      ...
      MPI_Allgatherv(sVl.data(), j * nsd, cm_mod::mpreal, sV.data(),
                     sCount.data(), disp.data(), cm_mod::mpreal, cm.com());
  ...
  if (btest(lBc.bType, enum_int(BoundaryConditionType::bType_flx))) {
    ...
    if (is_zero(tmp)) {
      tmp = 1.0;
      throw std::runtime_error("Face '" + lFa.name +
                               "' used for a BC has no non-zero node.");
    }
  }
...
#define n_debug_face_ini
  ...
  if (std::set<ElementType>{ElementType::TRI6, ElementType::QUD8,
                            ElementType::QUD9, ElementType::TET10,
...
//
  ...
  if (!flag) {
    ...
    // For higher order elements, use reduced order basis on mesh to project
    // element normals. Lumping method is used to project to face corners.
    // Normals at edge nodes are computed by simple interpolation from reduced
    // basis. Standard lumping using higher order basis could lead to spurious
    // errors.
    //
  } else {
    fsType fs;
    ...
    for (int e = 0; e < lFa.nEl; e++) {
      ...
      for (int a = 0; a < lFa.eNoN; a++) {
        ...
        if (b + 1 > lM.eNoN) {
          throw std::runtime_error(
              "Could not find matching face node on higher order mesh");
          // CALL STOPSIM()
        }
  ...
  for (int a = 0; a < lFa.nNo; a++) {
    ...
    if (utils::is_zero(sln)) {
      if (flag) {
        throw std::runtime_error("Skipping normal calculation for node " +
                                 std::to_string(a) + " in face '" + lFa.name +
                                 "'.");
        flag = false;
      }
    }
...
#define n_debug_fsi_ls_ini
  ...
  if (btest(lBc.bType, iBC_Dir)) {
    if (lBc.weakDir) {
      ...
      fsils_bc_create(com_mod.lhs, lsPtr, lFa.nNo, nsd, BcType::BC_TYPE_Dir,
                      gNodes, sVl);
    }
    ...
    if (btest(lBc.bType, iBC_res)) {
      ...
      lsPtr = lsPtr + 1;
      lBc.lsPtr = lsPtr;
      fsils_bc_create(com_mod.lhs, lsPtr, lFa.nNo, nsd, BcType::BC_TYPE_Neu,
                      gNodes, sVl);
    } else {
      lBc.lsPtr = -1;
    }
    ...
    fsils_bc_create(com_mod.lhs, lsPtr, nNo, nsd, BcType::BC_TYPE_Dir, gNodes,
                    sVl);
  } else {
  ...
  for (int e = 0; e < nEl; e++) {
    for (int a = 0; a < eNoN; a++) {
      ...
      for (int f = 0; f < nEl; f++) {
        ...
        if (lM.eIEN(a, e) == -1) {
          lM.sbc(a, e) = utils::ibset(
              lM.sbc(a, e), enum_int(BoundaryConditionType::bType_free));
        }
      }
    }
  }
    ...
    for (int a = 0; a < lM.eNoN; a++) {
      ...
      if (!bFlag) {
        if (!btest(lM.sbc(a, Ec), iBC_free)) {
          throw std::runtime_error(
              "BC detected on a non-boundary shell element. ");
        }
...
void shl_ini(const ComMod& com_mod, const CmMod& cm_mod, mshType& lM) {
  ...
  for (int a = 0; a < nNo; a++) {
    ...
    if (is_zero(Jac)) {
      if (flag) {
        throw std::runtime_error("Not a compatible cplBC_type");
        // wrn = "Skipping normal calculation of node "//a// " in mesh
        // <"//TRIM(lM.name)//">"
        flag = false;
      }
