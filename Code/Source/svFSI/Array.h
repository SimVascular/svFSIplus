...
class Array
{
  public:
    ...
    int msize() const { return size_ * sizeof(T); }
    ...
    {
      if (i >= size_) {
        throw std::runtime_error("[Array(i)] Index " + std::to_string(i) +
                                 " is out of bounds.");
      }
      return data_[i];
    }
    ...
    //
    Vector<T> col(const int col,
                  const std::array<int, 2>& range = {-1, -1}) const
    {
#ifdef Array_check_enabled
      check_index(0, col);
#endif
    ...
    //
    T* col_data(const int col) { return &data_[col * nrows_]; }
    ...
    //
    std::vector<T> values(const std::array<int, 2>& rows,
    ...
    //
    void set_col(const int col, const Vector<T>& values,
    ...
    /// @brief Set row values from a vector of values at the given column
    /// offset.
    //
    ...
    //
    void set_rows(const int start_row, const int end_row,
    ...
    Array<T> operator/(const Array<T>& array) const
    {
      ...
      if ((nrows_ != array.nrows_) || (ncols_ != array.ncols_)) {
        throw std::runtime_error(
            "[Array divide] Arrays number of columns or number of rows are not "
            "equal.");
      }
  ...
  private:
    ...
    void allocate(const int num_rows, const int num_cols)
    {
      ...
      if (data_ != nullptr) {
        // throw std::runtime_error(+"[Array] Allocating when data is not
        // nullptr.");
      }
    ...
    void check_index(const int row, const int col) const
    {
      ...
      if ((row < 0) || (row >= nrows_) || (col < 0) || (col >= ncols_)) {
        ...
        throw std::runtime_error(+"Index (row,col)=" + index_str +
                                 " is out of bounds for " + dims + " array.");
      }
    }
