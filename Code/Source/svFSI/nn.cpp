...
//
//   1) Set element properties: element type, number of Gauss integration
//   points, ...
//
...
void get_gip(const int insd, consts::ElementType eType, const int nG,
  ...
  try {
    ...
    throw std::runtime_error("No support for element etype " +
...
void get_gip(mshType& mesh) {
  try {
    ...
    throw std::runtime_error("No support for mesh etype " +
                             std::to_string(static_cast<int>(mesh.eType)) +
                             " in 'set_element_gauss_int_data'.");
  }
}
...
void get_gip(Simulation* simulation, faceType& face) {
  try {
    ...
    throw std::runtime_error("No support for face type " +
                             std::to_string(static_cast<int>(face.eType)) +
                             " in 'set_face_gauss_int_data'.");
  }
}
...
void get_gnn(const int insd, consts::ElementType eType, const int eNoN,
  ...
  try {
    ...
    throw std::runtime_error("[get_gnn] No support for element type " +
                             std::to_string(static_cast<int>(eType)) +
                             " in 'get_element_shape_data'.");
  }
}
...
void get_gn_nxx(const int insd, const int ind2, consts::ElementType eType,
  ...
  static std::set<ElementType> no_derivs{ElementType::NRB, ElementType::QUD4,
  ...
  try {
    ...
    throw std::runtime_error("[get_gn_nxx] No support for element type " +
                             std::to_string(static_cast<int>(eType)) +
                             " in 'get_element_2nd_derivs'.");
  }
}
...
void gnn(const int eNoN, const int nsd, const int insd, Array<double>& Nxi,
  ...
  if (insd == 1) {
    ...
    Jac =
        xXi(0, 0) * xXi(1, 1) * xXi(2, 2) + xXi(0, 1) * xXi(1, 2) * xXi(2, 0) +
        xXi(0, 2) * xXi(1, 0) * xXi(2, 1) - xXi(0, 0) * xXi(1, 2) * xXi(2, 1) -
        xXi(0, 1) * xXi(1, 0) * xXi(2, 2) - xXi(0, 2) * xXi(1, 1) * xXi(2, 0);
    ...
    ks(0, 0) =
        xiX(0, 0) * xiX(0, 0) + xiX(1, 0) * xiX(1, 0) + xiX(2, 0) * xiX(2, 0);
    ks(0, 1) =
        xiX(0, 1) * xiX(0, 0) + xiX(1, 1) * xiX(1, 0) + xiX(2, 1) * xiX(2, 0);
    ks(0, 2) =
        xiX(0, 2) * xiX(0, 0) + xiX(1, 2) * xiX(1, 0) + xiX(2, 2) * xiX(2, 0);
    ks(1, 1) =
        xiX(0, 1) * xiX(0, 1) + xiX(1, 1) * xiX(1, 1) + xiX(2, 1) * xiX(2, 1);
    ks(1, 2) =
        xiX(0, 1) * xiX(0, 2) + xiX(1, 1) * xiX(1, 2) + xiX(2, 1) * xiX(2, 2);
    ks(2, 2) =
        xiX(0, 2) * xiX(0, 2) + xiX(1, 2) * xiX(1, 2) + xiX(2, 2) * xiX(2, 2);
    ks(1, 0) = ks(0, 1);
    ks(2, 0) = ks(0, 2);
    ks(2, 1) = ks(1, 2);
    ...
    for (int a = 0; a < eNoN; a++) {
      ...
      Nx(1, a) = Nx(1, a) + Nxi(0, a) * xiX(0, 1) + Nxi(1, a) * xiX(1, 1) +
                 Nxi(2, a) * xiX(2, 1);
      Nx(2, a) = Nx(2, a) + Nxi(0, a) * xiX(0, 2) + Nxi(1, a) * xiX(1, 2) +
                 Nxi(2, a) * xiX(2, 2);
    }
  }
}
...
void gn_nxx(const int l, const int eNoN, const int nsd, const int insd,
  ...
  if (insd == 2) {
    ...
    K.set_row(0, {xXi(0, 0) * xXi(0, 0), xXi(1, 0) * xXi(1, 0),
                  t * xXi(0, 0) * xXi(1, 0)});
    K.set_row(1, {xXi(0, 1) * xXi(0, 0), xXi(1, 1) * xXi(1, 1),
                  t * xXi(0, 1) * xXi(1, 1)});
    K.set_row(2, {xXi(0, 0) * xXi(0, 1), xXi(1, 0) * xXi(1, 1),
                  xXi(0, 0) * xXi(1, 1) + xXi(0, 1) * xXi(1, 0)});
    ...
    for (int i = 0; i < 3; i++) {
      K.set_row(i, {xXi(0, i) * xXi(0, i), xXi(1, i) * xXi(1, i),
                    xXi(2, i) * xXi(1, i), t * xXi(0, i) * xXi(1, i),
                    t * xXi(1, i) * xXi(2, i), t * xXi(0, i) * xXi(2, i)});
    }
    ...
    K.set_row(
        3, {xXi(0, i) * xXi(0, j), xXi(1, i) * xXi(1, j), xXi(2, i) * xXi(2, j),
    ...
    j = 2;
    K.set_row(
        4, {xXi(0, i) * xXi(0, j), xXi(1, i) * xXi(1, j), xXi(2, i) * xXi(2, j),
            xXi(0, i) * xXi(1, j) + xXi(0, j) * xXi(1, i),
            xXi(1, i) * xXi(2, j) + xXi(1, j) * xXi(2, i),
            xXi(0, i) * xXi(2, j) + xXi(0, j) * xXi(2, i)});
    ...
    j = 2;
    K.set_row(
        5, {xXi(0, i) * xXi(0, j), xXi(1, i) * xXi(1, j), xXi(2, i) * xXi(2, j),
            xXi(0, i) * xXi(1, j) + xXi(0, j) * xXi(1, i),
            xXi(1, i) * xXi(2, j) + xXi(1, j) * xXi(2, i),
    ...
    for (int a = 0; a < eNoN; a++) {
      for (int i = 0; i < 6; i++) {
        B(i, a) = Nxi2(i, a) - Nx(0, a) * xXi2(0, i) - Nx(1, a) * xXi2(1, i) -
                  Nx(2, a) * xXi2(2, i);
      }
    }
...
void select_ele(const ComMod& com_mod, mshType& mesh) {
  ...
  try {
    ...
    throw std::runtime_error("[select_ele] No support for " +
                             std::to_string(mesh.eNoN) + " noded " +
...
void select_eleb(Simulation* simulation, mshType& mesh, faceType& face) {
  ...
  // Set element properties based on integration dimension and number of element
  // nodes.
  //
  try {
    set_face_element_props[face.eNoN](insd, face);
  } catch (const std::bad_function_call& exception) {
    throw std::runtime_error("No support for " + std::to_string(face.eNoN) +
                             " noded " + std::to_string(insd) +
                             "D elements in 'set_face_element_props'.");
  }
