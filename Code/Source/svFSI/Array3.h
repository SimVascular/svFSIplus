...
class Array3
{
  public:
    ...
    Array3(const int num_rows, const int num_cols, const int num_slices,
           bool row_major = true)
    {
    ...
    void allocate(const int num_rows, const int num_cols, const int num_slices,
    ...
    void check_index(const int i, const int j, const int k) const
    {
      ...
      if ((i < 0) || (i >= nrows_) or (j < 0) || (j >= ncols_) or (k < 0) ||
          (k >= nslices_)) {
        ...
        auto index_str = " " + std::to_string(i) + "," + std::to_string(j) +
                         "," + std::to_string(k) + " ";
        throw std::runtime_error("Index (i,j,k)=" + index_str +
    ...
    T* slice_data(const int slice) { return &data_[slice * slice_size_]; }
    ...
    Array<T> slice(const int slice) const
    {
      ...
      for (int col = 0; col < ncols_; col++) {
        for (int row = 0; row < nrows_; row++) {
          array_slice(row, col) =
              data_[row + col * nrows_ + slice * slice_size_];
        }
      }
    ...
    void set_row(const int col, const int slice,
    ...
    /// @brief Set the array values from an Array with the equivalent number of
    /// values.
    ///
    ...
    void set_values(Array<T>& rhs)
    {
      ...
      if (size_ != rhs_size) {
        throw std::runtime_error("The RHS size " + std::to_string(rhs_size) +
