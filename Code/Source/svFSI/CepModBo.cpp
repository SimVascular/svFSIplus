                         ...
                         double& gf)
{
  ...
  if (SL >= SLmin && SL <= SLmax) {
    SL = 0.5 * f0 + fc1 * cos(SL) + fs1 * sin(SL) + fc2 * cos(2.0 * SL) +
         fs2 * sin(2.0 * SL) + fc3 * cos(3.0 * SL) + fs3 * sin(3.0 * SL);
  } else {
    SL = 0.0;
  }
                    ...
                    Vector<double>& f, const double fext, Vector<double>& RPAR)
{
  ...
  double taum_w = taum_w1[i] + 0.5 * (taum_w2[i] - taum_w1[i]) *
                                   (1.0 + tanh(km_w[i] * (u - um_w[i])));
  double tau_so = tau_so1[i] + 0.5 * (tau_so2[i] - tau_so1[i]) *
                    ...
                    Array<double>& JAC)
{
  ...
  // Define additional constants
  double taum_v = (1.0 - H_umv) * taum_v1[i] + H_umv * taum_v2[i];
  double taum_w = taum_w1[i] + 0.50 * (taum_w2[i] - taum_w1[i]) *
                                   (1.0 + tanh(km_w[i] * (u - um_w[i])));
  double tau_so = tau_so1[i] + 0.50 * (tau_so2[i] - tau_so1[i]) *
                                   (1.0 + tanh(k_so[i] * (u - u_so[i])));
  double tau_s = (1.0 - H_uw) * tau_s1[i] + H_uw * tau_s2[i];
  double tau_o = (1.0 - H_uo) * tau_o1[i] + H_uo * tau_o2[i];
  double v_inf = (1.0 - H_umv);
