...
/// subroutine.
//
using SetEquationPropertiesMapType = std::map<
    consts::EquationType,
    std::function<void(Simulation*, EquationParameters*, eqType&,
                       EquationProps&, EquationOutputs&, EquationNdop&)>>;
...
SetEquationPropertiesMapType set_equation_props = {
    ...
    //---------------------------//
    //
    {consts::EquationType::phys_CEP,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
        EquationProps& propL, EquationOutputs& outPuts,
        EquationNdop& nDOP) -> void {
    ...
    {consts::EquationType::phys_CMM,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
       ...
       bool pstEq =
           eq_params->prestress.defined() && eq_params->prestress.value();
       ...
       if (eq_params->initialize.defined()) {
         ...
         std::transform(init_str.begin(), init_str.end(), init_str.begin(),
                        ::tolower);
         if (std::set<std::string>{"inflate", "inf"}.count(init_str) != 0) {
           com_mod.pstEq = false;
         } else if (std::set<std::string>{"prestress", "prest"}.count(
                        init_str) != 0) {
           com_mod.pstEq = true;
         } else {
       ...
       if (eq_params->variable_wall_properties.defined()) {
         ...
         auto file_path = eq_params->variable_wall_properties
                              .wall_properties_file_path.value();
       ...
       if (!com_mod.cmmInit) {
         ...
         nDOP = {12, 4, 3, 0};
         outPuts = {OutputType::out_velocity,   OutputType::out_pressure,
                    OutputType::out_WSS,        OutputType::out_displacement,
                    OutputType::out_energyFlux, OutputType::out_traction,
                    OutputType::out_vorticity,  OutputType::out_vortex,
                    OutputType::out_strainInv,  OutputType::out_viscosity,
                    OutputType::out_divergence, OutputType::out_acceleration};
    ...
    {consts::EquationType::phys_fluid,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
       ...
       outPuts = {OutputType::out_velocity,    OutputType::out_pressure,
                  OutputType::out_WSS,         OutputType::out_traction,
                  OutputType::out_vorticity,   OutputType::out_vortex,
                  OutputType::out_strainInv,   OutputType::out_energyFlux,
                  OutputType::out_viscosity,   OutputType::out_divergence,
    ...
    //---------------------------//
    //
    {consts::EquationType::phys_heatF,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
        EquationProps& propL, EquationOutputs& outPuts,
        EquationNdop& nDOP) -> void {
       using namespace consts;
    ...
    //---------------------------//
    //
    {consts::EquationType::phys_heatS,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
        EquationProps& propL, EquationOutputs& outPuts,
        EquationNdop& nDOP) -> void {
       using namespace consts;
    ...
    {consts::EquationType::phys_FSI,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
       ...
       // Set the possible equations for fsi: fluid (required),
       // struct/ustruct/lElas
       ...
       nDOP = {22, 4, 2, 0};
       outPuts = {OutputType::out_velocity,     OutputType::out_pressure,
                  OutputType::out_displacement, OutputType::out_mises,

                  OutputType::out_WSS,          OutputType::out_traction,
                  OutputType::out_vorticity,    OutputType::out_vortex,
                  OutputType::out_strainInv,    OutputType::out_energyFlux,
                  OutputType::out_viscosity,    OutputType::out_absVelocity,
                  OutputType::out_stress,       OutputType::out_cauchy,
                  OutputType::out_strain,       OutputType::out_jacobian,
                  OutputType::out_defGrad,      OutputType::out_integ,
                  OutputType::out_fibDir,       OutputType::out_fibAlign,
    ...
    //---------------------------//
    //
    {consts::EquationType::phys_lElas,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
        EquationProps& propL, EquationOutputs& outPuts,
        EquationNdop& nDOP) -> void {
       using namespace consts;
       ...
       if (eq_params->prestress.defined() && eq_params->prestress.value()) {
         ...
         nDOP = {8, 2, 0, 0};
         outPuts = {OutputType::out_displacement, OutputType::out_mises,
                    OutputType::out_stress,       OutputType::out_strain,
                    OutputType::out_velocity,     OutputType::out_acceleration,
    ...
    {consts::EquationType::phys_mesh,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
       ...
       nDOP = {3, 1, 0, 0};
       outPuts = {OutputType::out_displacement, OutputType::out_velocity,
                  OutputType::out_acceleration};
    ...
    //---------------------------//
    //
    {consts::EquationType::phys_shell,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
        EquationProps& propL, EquationOutputs& outPuts,
        EquationNdop& nDOP) -> void {
       using namespace consts;
       ...
       outPuts = {OutputType::out_displacement, OutputType::out_stress,
                  OutputType::out_strain,       OutputType::out_jacobian,
                  OutputType::out_defGrad,      OutputType::out_velocity,
                  OutputType::out_integ,        OutputType::out_CGstrain,
    ...
    {consts::EquationType::phys_stokes,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
       ...
       nDOP = {8, 2, 3, 0};
       outPuts = {OutputType::out_velocity,  OutputType::out_pressure,
                  OutputType::out_WSS,       OutputType::out_vorticity,
                  OutputType::out_traction,  OutputType::out_strainInv,
                  OutputType::out_viscosity, OutputType::out_divergence};
    ...
    //---------------------------//
    //
    {consts::EquationType::phys_struct,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
        EquationProps& propL, EquationOutputs& outPuts,
        EquationNdop& nDOP) -> void {
       using namespace consts;
       ...
       if (eq_params->prestress.defined() && eq_params->prestress.value()) {
         nDOP = {4, 2, 0, 0};
         outPuts = {OutputType::out_displacement, OutputType::out_stress,
                    OutputType::out_cauchy, OutputType::out_strain};
    ...
    //---------------------------//
    //
    {consts::EquationType::phys_ustruct,
     [](Simulation* simulation, EquationParameters* eq_params, eqType& lEq,
        EquationProps& propL, EquationOutputs& outPuts,
        EquationNdop& nDOP) -> void {
       using namespace consts;
       ...
       outPuts = {OutputType::out_displacement, OutputType::out_mises,
                  ...
                  OutputType::out_defGrad,      OutputType::out_integ,
                  OutputType::out_fibDir,       OutputType::out_fibAlign,
                  OutputType::out_velocity,     OutputType::out_pressure,
                  OutputType::out_acceleration, OutputType::out_divergence};
