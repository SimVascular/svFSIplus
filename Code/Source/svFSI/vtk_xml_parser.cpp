...
// vtkXMLParser.f90.
//
// Volume mesh and face data is read using VTK readers to read VTU and VTP
// files. Data read in is stored directly into mshType and faceType objects.
...
std::map<unsigned char, int> vtk_cell_to_elem{
    {VTK_HEXAHEDRON, 8}, {VTK_LINE, 2},     {VTK_QUAD, 4},
    {VTK_TETRA, 4},      {VTK_TRIANGLE, 3}, {VTK_WEDGE, 6}};
...
///
/// Note: The face.IEN array is allocated as in the Fortran code as (face.eNoN,
/// face.nEl).
//
...
#define n_debug_store_element_conn
  ...
  std::cout << "[store_element_conn(polydata)] ========== "
               "store_element_conn(polydata) ========="
            << std::endl;
#endif
...
#ifdef debug_store_element_conn
  std::cout << "[store_element_conn(polydata)] np_elem: " << np_elem
            << std::endl;
  std::cout << "[store_element_conn(polydata)] cell type: "
            << vtk_polydata->GetCellType(0) << std::endl;
#endif
...
void store_element_conn(vtkSmartPointer<vtkPolyData> vtk_polydata,
  ...
  std::cout << "[store_element_conn(polydata,mesh)] ========== "
               "store_element_conn(polydata,mesh) ========="
            << std::endl;
#endif
...
#ifdef debug_store_element_conn
  std::cout << "[store_element_conn(polydata,mesh)] num_elems: " << num_elems
            << std::endl;
  std::cout << "[store_element_conn(polydata,mesh)] np_elem: " << np_elem
            << std::endl;
#endif
...
///
/// Note: The mesh.gIEN array is allocated as in the Fortran code as (np_elem,
/// num_elems).
//
void store_element_conn(vtkSmartPointer<vtkUnstructuredGrid> vtk_ugrid,
  ...
  std::cout << "[store_element_conn(ugrid)] ========== "
               "store_element_conn(ugrid) ========="
            << std::endl;
#endif
  vtkSmartPointer<vtkUnsignedCharArray> cell_types =
      vtk_ugrid->GetCellTypesArray();
  auto num_elems = vtk_ugrid->GetNumberOfCells();
#ifdef debug_store_element_conn
  std::cout << "[store_element_conn(ugrid)] num_elems: " << num_elems
            << std::endl;
#endif
  ...
  std::cout << "[store_element_conn] Number of elements: " << num_elems
            << std::endl;
  std::cout << "[store_element_conn] Number of nodes per element: " << np_elem
            << std::endl;
#endif
  ...
  for (int i = 0; i < num_elems; i++) {
    ...
    if (num_cell_pts != np_elem) {
      throw std::runtime_error(
          "[store_element_conn] Error in VTK mesh data for mesh '" + mesh.name +
          "'.");
    }
    for (int j = 0; j < num_cell_pts; j++) {
      auto id = cell->PointIds->GetId(j);
...
void store_element_ids(vtkSmartPointer<vtkUnstructuredGrid> vtk_ugrid,
  ...
  if (elem_ids == nullptr) {
    throw std::runtime_error("No '" + ELEMENT_IDS_NAME +
                             "' data found in VTK mesh.");
  }
...
void store_nodal_ids(vtkSmartPointer<vtkUnstructuredGrid> vtk_ugrid,
  ...
  for (int i = 0; i < num_nodes; i++) {
    ...
    // std::cout << "[store_nodal_ids] mesh.gN(" << i << "): " << mesh.gN(i) <<
    // std::endl;
  }
}
...
void store_nodal_ids(vtkSmartPointer<vtkPolyData> vtk_polydata,
  ...
  for (int i = 0; i < num_nodes; i++) {
    ...
    // std::cout << "[store_nodal_ids] face.gN(" << i << "): " << face.gN(i) <<
    // std::endl;
  }
}
...
///   data_name - The name of the VTK Cell Data Array storing fiber direction
///   data idx - The id used to identify a fiber file (0, 1, ..., number of
///   fiber files - 1) mesh - The mesh to store the data into.
//
void load_fiber_direction_vtu(const std::string& file_name,
  ...
  std::cout << "[load_fiber_direction_vtu] ===== "
               "vtk_xml_parser::load_fiber_direction_vtu ===== "
            << std::endl;
#endif
  ...
  if (mesh.gnEl != num_elems) {
    throw std::runtime_error("The number of elements (" +
                             ...
                             std::to_string(mesh.gnEl) +
                             ") for the mesh named '" + mesh.name + "'.");
  }

  // Get the 3-component fiber orientation data.
  auto fiber_data = vtkDoubleArray::SafeDownCast(
      vtk_ugrid->GetCellData()->GetArray(data_name.c_str()));
  if (fiber_data == nullptr) {
    throw std::runtime_error("No '" + data_name +
                             "' data found in the fiber direction VTK file '" +
                             file_name + "'");
  }
  ...
  for (int e = 0; e < mesh.gnEl; e++) {
    ...
    // std::cout << "[load_fiber_direction_vtu] e mesh.fN: " << e+1 << " " <<
    // mesh.fN.col(e) << std::endl;
  }
}
...
#define n_debug_load_vtu
  ...
  std::cout << "[load_vtu] Number of nodes per element: " << np_elem
            << std::endl;
#endif
