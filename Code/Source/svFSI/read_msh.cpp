...
//
std::map<consts::ElementType, std::function<void(mshType&)>>
    check_element_conn = {{consts::ElementType::LIN1, check_line_conn},
...
#define n_debug_calc_elem_ar
  ...
  for (int i = 0; i < 5; i++) {
    tmp[i] =
        100.0 * static_cast<double>(bins[i]) / static_cast<double>(lM.gnEl);
  }
      ...
      if (Jac(e) < 0.0) {
        ...
        if (cPhys != Equation_fluid) {
          throw std::runtime_error(
              "[calc_elem_jac] Negative Jacobian in non-fluid domain.");
        }
      }
    }
  }
  ...
  if (!rflag) {
    ...
    */
    ...
    if (!com_mod.rmsh.isReqd) {
      throw std::runtime_error(
          "[calc_elem_jac] Unexpected behavior! Mesh is DISTORTED.");
    }
  }
#ifdef debug_calc_elem_jac
  dmsg << "Done "
...
#define n_debug_calc_elem_skew
  ...
  if (lM.eType != ElementType::TET4 && lM.eType != ElementType::TRI3) {
    std::cout << "WARNING: Skewness is computed for TRI and TET elements only."
              << std::endl;
    return;
  }
...
#define n_debug_calc_mesh_props
  ...
  if (rmsh.isReqd && std::count(rmsh.flag.begin(), rmsh.flag.end(), true) !=
                         rmsh.flag.size()) {
  ...
  MPI_Allgather(rmsh_flag, nMesh, cm_mod::mplog, gFlag.data(), nMesh,
                cm_mod::mplog, com_mod.cm.com());
...
#define n_debug_calc_nbc
  ...
  for (int e = 0; e < face.nEl; e++) {
    ...
    if (Ec > num_elems) {
      throw std::runtime_error(msg + "element ID " + std::to_string(Ec) +
                               " exceeds the number of elements (" +
                               std::to_string(num_elems) + ") in the mesh.");
    } else if (Ec < 0) {
      throw std::runtime_error(msg + "element ID " + std::to_string(Ec) +
                               " is < 0.");
    }
    ...
    for (int a = 0; a < face.eNoN; a++) {
      ...
      // dmsg << "Ac: " << Ac;
      if (Ac > num_nodes) {
        throw std::runtime_error(msg + "element " + std::to_string(e) +
                                 " has a node " + std::to_string(Ac) +
                                 " that exceeds the number of nodes (" +
      ...
      if (std::find(elem_conn.begin(), elem_conn.end(), Ac) ==
          elem_conn.end()) {
        throw std::runtime_error(
            msg + "element " + std::to_string(e) + " has a node " +
            std::to_string(Ac) +
            " that does not belong to its parent element " +
            std::to_string(Ec) + ".");
      }
...
void check_ien(Simulation* simulation, mshType& mesh) {
  ...
  for (int e = 0; e < num_elems; e++) {
    for (int a = 0; a < teNoN; a++) {
      ...
      if ((Ac >= num_nodes) || (Ac < 0)) {
        throw std::runtime_error("Element " + std::to_string(e) +
                                 " contains a node " + std::to_string(Ac) +
                                 " that is not within 0 and " +
  ...
  for (int Ac = 0; Ac < num_nodes; Ac++) {
    if (incNodes(Ac) == 0) {
      throw std::runtime_error("Node " + std::to_string(Ac) +
                               " is not used by any element.");
    }
  }
  ...
  try {
    ...
    throw std::runtime_error("[check_ien] eType " +
...
void check_tet_conn(mshType& mesh) {
  ...
  int num_elems = mesh.gnEl;
  int num_reorder = 0;
  // std::cout << "[check_tet_conn] mesh.gIEN.data(): " << mesh.gIEN.data() <<
  // std::endl; std::cout << "[check_tet_conn] mesh.x.data(): " << mesh.x.data()
  // << std::endl;
  ...
  for (int e = 0; e < num_elems; e++) {
    ...
    if (sn == 1) {
      ...
      num_reorder += 1;
      if (e == 0) {
        // std::cout << "[check_tet_conn] Reorder element connectivity for
        // element " << e << std::endl;
      }
      // std::cout << "[check_tet_conn] Reorder element connectivity for element
      // " << e << std::endl;
    } else if (sn == 0) {
      throw std::runtime_error("Tet element " + std::to_string(e) +
                               " is distorted.");
    }
  ...
  // std::cout << "[check_tet_conn] Reorder " << num_reorder << " element
  // connectivity from " << num_elems << "  elements." << std::endl;
}
...
void check_wedge_conn(mshType& mesh) {
  ...
  for (int e = 0; e < num_elems; e++) {
    ...
    if (sn == -1) {
      ...
      b = 5;
      if (e == 0) {
        // std::cout << "[check_wedge_conn] Reorder element connectivity." <<
        // std::endl;
      }
    } else if (sn == 0) {
      throw std::runtime_error("Element " + std::to_string(e) +
...
void load_var_ini(Simulation* simulation, ComMod& com_mod) {
  ...
  for (int iM = 0; iM < com_mod.nMsh; iM++) {
    ...
    if (mesh_param->initial_velocities_file_path.defined()) {
      ...
      int data_series = 0;
      vtk_xml::read_vtu_pdata(cTmp, "Velocity", com_mod.nsd, data_comp,
                              data_series, com_mod.msh[iM]);
    }
  }
  ...
  for (int iM = 0; iM < com_mod.nMsh; iM++) {
    ...
    if (mesh_param->initial_displacements_file_path.defined()) {
      ...
      int data_comp = com_mod.nsd;
      int data_series = 0;
      vtk_xml::read_vtu_pdata(cTmp, "Displacement", com_mod.nsd, data_comp,
                              data_series, com_mod.msh[iM]);
    }
  }
...
#define n_debug_match_faces
  ...
  if (cnt != lFa.nNo) {
    throw std::runtime_error("Failed to find matching nodes between faces " +
                             lFa.name + " and " + pFa.name + ".");
  }

  if (lPrj.n == 0) {
    throw std::runtime_error("No matching nodes between faces " + lFa.name +
                             " and " + pFa.name + ".");
  }

  if (lPrj.n / 2 != lFa.nNo) {
    throw std::runtime_error("Mismatch of nodes between faces " + lFa.name +
                             " and " + pFa.name + ".");
  }
}
...
//
      ...
      for (int iFa = 0; iFa < mesh.nFa; iFa++) {
        ...
        for (int i = 0; i < iM; i++) {
          for (int j = 0; j < com_mod.msh[i].nFa; j++) {
            if ((ctmp == com_mod.msh[i].fa[j].name) &&
                ((i != iM || j != iFa))) {
              throw std::runtime_error("The face name '" + ctmp +
                                       "' is duplicated.");
            }
          }
        }
      ...
      if (mesh.qmTET4 < (1.0 / 4.0) || mesh.qmTET4 > 1.0) {
        throw std::runtime_error(
            "Quadrature_modifier_TET4 must be in the range [1/4, 1].");
      }
    }
    ...
    for (int iM = 0; iM < com_mod.nMsh; iM++) {
      ...
      if (mesh.lShl) {
        if (mesh.eType != consts::ElementType::NRB &&
            mesh.eType != consts::ElementType::TRI3) {
          throw std::runtime_error(
              "Shell elements must be triangles or C1-NURBS.");
        }
        ...
        if (mesh.eType == consts::ElementType::NRB) {
          for (int i = 0; i < com_mod.nsd - 1; i++) {
            if (mesh.bs[i].p <= 1) {
              throw std::runtime_error(
                  "NURBS for shell elements must have be p > 1.");
            }
          }
        }
        ...
        if (mesh.eType == consts::ElementType::TRI3) {
          if (!com_mod.cm.seq()) {
            throw std::runtime_error(
                "Shells with linear triangles should be run sequentially.");
          }
        }
      }
    }
    ...
    for (int iM = 0; iM < com_mod.nMsh; iM++) {
      auto& mesh = com_mod.msh[iM];
      if (mesh.lFib) {
        if (mesh.eType != consts::ElementType::LIN1 &&
            mesh.eType != consts::ElementType::LIN2) {
          throw std::runtime_error(
              "Fiber elements must be either linear quadratic.");
        }
      }
    }
  ...
  if (avNds.n != 0) {
    throw std::runtime_error("There are " + std::to_string(avNds.n) +
                             " nodes not associated other faces.");
  }
...
//
  ...
  for (int iM = 0; iM < com_mod.nMsh; iM++) {
    ...
    if (mesh_param->domain_file_path.defined()) {
      /*
      if (rmsh.isReqd) {
        throw std::runtime_error("Variable domain properties are not allowed
      with remeshing.");
      }
      */
...
#endif
      if ((domain_file_path.find(".vtp") != std::string::npos) ||
          (domain_file_path.find(".vtu") != std::string::npos)) {
        set_dmn_id_vtk(simulation, com_mod.msh[iM], domain_file_path,
                       "DOMAIN_ID");
      } else {
      ...
      if (num_paths != 0) {
        ...
        if (num_dirs != 0) {
          ...
          for (int i = 0; i < com_mod.msh[iM].nFn; i++) {
            ...
            double rtmp =
                sqrt(fibN[0] * fibN[0] + fibN[1] * fibN[1] + fibN[2] * fibN[2]);
  ...
  for (int iM = 0; iM < com_mod.nMsh; iM++) {
    ...
    if (mesh_param->prestress_file_path.defined()) {
      ...
      if (rmsh.isReqd) {
        throw std::runtime_error("Prestress is currently not allowed with
      remeshing.");
      }
      */
      ...
      vtk_xml::read_vtu_pdata(cTmp, "Stress", com_mod.nsd, com_mod.nsymd, 0,
                              com_mod.msh[iM]);
    }
  }
      ...
      switch (cntctM.cType) {
        case consts::ContactModelType::cntctM_penalty:
          ...
          if (cntctM.c < cntctM.h) {
            throw std::runtime_error(
                "The contact Closest_gap_to_activate_penalty " +
                std::to_string(cntctM.c) +
                " must be > the desired separation " +
                std::to_string(cntctM.h) + ".");
          }
          break;

        default:
          throw std::runtime_error("Contact model '" + contact_model +
                                   "' is not implemented.");
          break;
      }
    }
...
#define n_debug_set_dmn_id_ff
  ...
  if (!domain_ids_file.is_open()) {
    throw std::runtime_error("Failed to open the domain IDs file '" +
                             file_name + "'.");
  }
  ...
  for (int e = 0; e < lM.gnEl; e++) {
    if (!domain_ids_file.good()) {
      throw std::runtime_error(
          "The domain IDs file '" + file_name +
          "' contains too few values than the number of elements.");
      break;
    }
    ...
    if (!(str_stream >> domain_id)) {
      throw std::runtime_error("Domain ID integer value '" + str_value +
                               "' found in the domain IDs file '" + file_name +
                               "'.");
    }

    if (domain_id <= 0) {
      throw std::runtime_error("The domain ID '" + str_value +
                               "' found in the domain IDs file '" + file_name +
                               "' is <= 0.");
    }
    ...
    if (num_bits > btSiz) {
      throw std::runtime_error("The domain ID '" + str_value +
                               "' found in domain IDs file '" + file_name +
                               "' is too large.");
    }
...
#define n_debug_set_projector
  ...
  for (auto& params : simulation->parameters.projection_parameters) {
    ...
    while (true) {
      ...
      if (i == -1) {
        if (j == -1) {
          // Since neither of them have value add a new node and both of them to
          // the stack.
