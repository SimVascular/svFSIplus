...
#define n_debug_gmres
  ...
  for (int l = 0; l < ls.mItr; l++) {
    ...
    if (l == 0) {
      ...
      spar_mul::fsils_spar_mul_vv(lhs, lhs.rowPtr, lhs.colPtr, dof, Val, X,
                                  u_slice);
    ...
    for (auto& face : lhs.face) {
      if (face.coupledFlag) {
        ...
        add_bc_mul::add_bc_mul(lhs, BcopType::BCOP_TYPE_PRE, dof, unCondU,
                               u_slice);
        break;
      }
    }
    ...
    for (int i = 0; i < ls.sD; i++) {
      ...
      spar_mul::fsils_spar_mul_vv(lhs, lhs.rowPtr, lhs.colPtr, dof, Val,
                                  u_slice, u_slice_1);

      add_bc_mul::add_bc_mul(lhs, BcopType::BCOP_TYPE_ADD, dof, u_slice,
                             u_slice_1);
...
#define n_debug_gmres_s
  ...
  for (int l = 0; l < ls.mItr; l++) {
    ...
    for (int i = 0; i < ls.sD; i++) {
      ...
      spar_mul::fsils_spar_mul_ss(lhs, lhs.rowPtr, lhs.colPtr, Val, u_col,
                                  u_col_1);
